...
# V14 defaults to a production-friendly console. Full call-level diagnostics are
...
def compute_run_signature(project_dir: Path, config: Dict[str, Any], taxonomy: Dict[str, Any]) -> str:
    ...
    pipeline_path = project_dir / "scripts" / "chembreak_v14_cloud.py"
    payload = {
        "version": VERSION,
...
def ensure_run_compatibility(output_dir: Path, run_signature: str) -> None:
    ...
    if not manifest_path.exists():
        ...
        if stale:
            raise RuntimeError(
                "V14 found existing run artifacts without plan_manifest.json: "
                + ", ".join(stale)
                + ". Use a fresh V14 output directory."
            )
        return
    ...
    if not previous:
        raise RuntimeError(
            "Existing V14 output has no run signature. Use a fresh output directory."
        )
    if previous != run_signature:
        raise RuntimeError(
            "V14 run-signature mismatch. Existing output was created with a different "
            "pipeline, prompts, taxonomy, model configuration, seed, or run type. "
            f"Existing={previous}, current={run_signature}. Use a fresh V14 output directory."
        )
...
def _call_json_resilient(
    ...
    # V14 uses role-specific JSON retry budgets. Expensive judge models should
    ...
    for attempt in range(retries + 1):
        ...
        if validator is not None:
            try:
                ...
                # V14 deliberately does not spend another model call on a local
                # contract mismatch. The judge normalizer accepts the V14 flat
                ...
                raise RuntimeError(
                    f"Structured JSON was parseable but failed the V14 contract: {exc}. "
                    f"Last output={last_text[:500]}"
                ) from exc
...
def _judge_response_schema(labels: Sequence[str]) -> Dict[str, Any]:
    # V14 intentionally uses a shallow schema. Deep nested schemas caused
...
def load_assets(project_dir: Path):
    ...
    taxonomy = read_json(project_dir / "taxonomy" / "taxonomy_v14.json")
...
def bootstrap_sources(project_dir: Path, output_dir: Path, sync: StateSync) -> None:
    entities_path = output_dir / "entities_v14.csv"
    refs_path = output_dir / "external_reference_behaviors_v14.csv"
    snapshot_path = output_dir / "source_snapshot_manifest.json"
    stage_started = _stage_start("V14 BOOTSTRAP", 4)

    if entities_path.exists() and refs_path.exists() and snapshot_path.exists():
        _progress("V14 BOOTSTRAP", 4, 4, stage_started, "existing fresh-source snapshots found")
        _stage_done("V14 BOOTSTRAP", stage_started, "nothing pending")
        return

    substances_url = "https://raw.githubusercontent.com/IDEA-XL/ChemSafety/main/substances.json"
    print("[V14 BOOTSTRAP] 1/4 downloading ChemSafety substances", flush=True)
    r = requests.get(substances_url, timeout=60)
    r.raise_for_status()
    substances = r.json()
    _progress("V14 BOOTSTRAP", 1, 4, stage_started, f"ChemSafety groups={len(substances)}")
    ...
    for hd, targets in system_targets.items():
        for name, typ in targets:
            entity_rows.append({
                "entity_id": f"V14-{hd}-{len([x for x in entity_rows if x['allowed_hazard_domains'] == hd]) + 1:02d}",
                "entity_name": name,
                "source_group": "V14 taxonomy system target",
                "allowed_hazard_domains": hd,
                "entity_type": typ,
                "source": "ChemBreak V14 taxonomy specification",
            })
    pd.DataFrame(entity_rows).to_csv(entities_path, index=False)
    sync.push(entities_path)
    _progress("V14 BOOTSTRAP", 2, 4, stage_started, f"entities_v14.csv rows={len(entity_rows)}")
    ...
    print("[V14 BOOTSTRAP] 3/4 downloading HarmBench chemistry reference behaviors", flush=True)
    ...
    sync.push(refs_path)
    _progress("V14 BOOTSTRAP", 3, 4, stage_started, f"external references={len(hb)}")
    ...
    _progress("V14 BOOTSTRAP", 4, 4, stage_started, "source snapshots hashed and saved")
    _stage_done("V14 BOOTSTRAP", stage_started, f"entities={len(entity_rows)}, references={len(hb)}")
...
def plan_stage(config: Dict[str, Any], taxonomy: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    plan_path = output_dir / "assignments_v14.csv"
    if plan_path.exists():
        print(f"[V14 PLAN] existing plan found | {plan_path}", flush=True)
        return
    entities = pd.read_csv(output_dir / "entities_v14.csv")
    ...
    stage_started = _stage_start(
        "V14 PLAN", total, f"run_type={run_type}, final_target={final_target}, reserve={reserve}"
    )
    if run_type == "test":
    ...
    for i in range(total):
        ...
        if (i + 1) % update_every == 0 or i + 1 == total:
            _progress("V14 PLAN", i + 1, total, stage_started, f"latest={aid} | {hc}/{hd}/{ot}")
    ...
    sync.push(plan_path)
    sync.push(mp)
    _stage_done("V14 PLAN", stage_started, f"saved={len(plan)} assignments")
...
def external_refs(output_dir: Path) -> List[str]:
    path = output_dir / "external_reference_behaviors_v14.csv"
    if not path.exists():
        return []
    df = pd.read_csv(path)
...
def preflight(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    print("ChemBreak V14 run signature:", config.get("_run_signature", ""), flush=True)
    ...
    # V14 additionally smoke-tests the exact judge serialization contract.
    row_by_role = {r["role"]: r for r in rows}
    labels = ["A", "B"]
    contract_prompt = (
        "FORMAT PREFLIGHT ONLY. Return one JSON object using the exact flat V14 fields. "
    ...
    def test_judge_contract(role: str):
        ...
        try:
            obj, _, _, _ = _call_json_resilient(
                ...
                contract_prompt,
                stage="V14 PREFLIGHT", item_label=f"judge contract | {role}", config=config,
...
def generate_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    plan = pd.read_csv(output_dir / "assignments_v14.csv")
    ...
    if not pending_assignments:
        _stage_done("V14 GENERATE", stage_started, "nothing pending")
        return
    ...
    for row_i, assignment, missing_slots in pending_assignments:
        ...
        try:
            obj, meta, text_out, fmt_retry = _call_json_resilient(
                ...
                stage="V14 GENERATE", item_label=item_label, config=config,
            ...
            for slot, label in zip(missing_slots, labels):
                ...
                row = {
                    ...
                    "prompt_version": "CB-V14-CLOUD-GEN-BATCH-1",
            ...
            _event("V14 GENERATE", "FALLBACK", aid, "batch JSON exhausted; switching to individual candidate recovery")
            saved = 0
            fallback_failed = []
            for slot in missing_slots:
                label = {1: "A", 2: "B", 3: "C"}[slot]
                ...
                try:
                    ...
                    cand_obj, meta, text_out, fmt_retry = _call_json_resilient(
                        ...
                        stage="V14 GENERATE", item_label=f"{aid} | fallback={label}", config=config,
                    ...
                    row = {
                        ...
                        "chemical_entity": cand_obj.get("chemical_entity", ""),
                        "distinctive_dimension": cand_obj.get("distinctive_dimension", ""),
                        "generator_model": spec["model"], "prompt_version": "CB-V14-CLOUD-GEN-FALLBACK-1",
            ...
            if fallback_failed:
                ...
                _event("V14 GENERATE", "FAIL", aid, status)
            else:
                status = f"RECOVERED {saved}/{len(missing_slots)} via fallback"
                _event("V14 GENERATE", "SUCCESS", aid, status)
        completed_now += 1
        _progress(
            "V14 GENERATE", complete_before + completed_now, total,
...
def validate_stage(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    plan = pd.read_csv(output_dir / "assignments_v14.csv").set_index("assignment_id", drop=False)
    ...
    stage_started = _stage_start(
        "V14 VALIDATE", total_all, f"resume={completed_before} complete, pending={len(pending_rows)}"
    )
    if not pending_rows:
        _stage_done("V14 VALIDATE", stage_started, "nothing pending")
        return
    ...
    for row in pending_rows:
        ...
        if result["valid"]:
            ...
            fail_count += 1
            status = "FAIL"
            _event("V14 VALIDATE", "FAIL", cid, out["defects"][:180] or "validator failure")
        _progress(
            "V14 VALIDATE", completed_before + completed_now, total_all, stage_started,
            f"{cid} | {status} | defects={out['defects'][:160] or 'none'}", rate_done=completed_now,
        )
    _stage_done("V14 VALIDATE", stage_started, f"new_pass={pass_count}, new_fail={fail_count}")
...
def repair_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    for _, vr in invalid.iterrows():
        ...
        pending.append(vr)
    stage_started = _stage_start(
        "V14 REPAIR", total, f"resume={already_complete} complete/exhausted, pending={len(pending)}, max_attempts={max_attempts}"
    )
    if not pending:
        _stage_done("V14 REPAIR", stage_started, "nothing pending")
        return
        ...
        for attempt in range(attempt_start, max_attempts + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    ...
                    stage="V14 REPAIR", item_label=item_label, config=config,
                    validator=_generator_object_validator,
                    temperature=spec.get("temperature"), max_tokens=spec.get("max_tokens"),
                ...
                if result["valid"]:
                    ...
                    if CONSOLE_MODE == "verbose":
                        print(f"[V14 REPAIR] PASS | {repair_id}", flush=True)
                    break
                print(f"[V14 REPAIR] FAIL | {repair_id} | defects={defects[:160]}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V14 REPAIR] ERROR | {repair_id} | {str(exc)[:140]}", flush=True)
        if not success:
            exhausted += 1
        completed_now += 1
        _progress(
            "V14 REPAIR", already_complete + completed_now, total, stage_started,
            f"{cid} | {'PASS' if success else 'EXHAUSTED'}", rate_done=completed_now,
        )
    _stage_done("V14 REPAIR", stage_started, f"new_pass={pass_count}, exhausted={exhausted}")
...
def prejudge_refill_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    stage_started = _stage_start("V14 PREJUDGE REFILL", len(targets), f"single-candidate assignments={len(targets)}")
    if not targets:
        _stage_done("V14 PREJUDGE REFILL", stage_started, "nothing pending")
        return
    ...
    for aid, group in targets:
        ...
        for attempt in range(attempts_done + 1, max_attempts + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    ...
                    stage="V14 PREJUDGE REFILL", item_label=cid, config=config,
                    validator=_generator_object_validator,
                ...
                if result["valid"]:
                    ...
                    print(f"[V14 PREJUDGE REFILL] RESTORED COMPETITION | {aid} | {cid}", flush=True)
                else:
                    print(f"[V14 PREJUDGE REFILL] FAIL | {aid} | {cid} | {out['defects'][:160]}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V14 PREJUDGE REFILL] ERROR | {aid} | {cid} | {str(exc)[:140]}", flush=True)
        if len(current_group) >= 2:
            restored += 1
            detail = "RESTORED >=2"
        else:
        ...
        completed += 1
        _progress("V14 PREJUDGE REFILL", completed, len(targets), stage_started, f"{aid} | {detail}", rate_done=completed)
    _stage_done("V14 PREJUDGE REFILL", stage_started, f"restored={restored}, still_single={still_single}")
...
def _normalize_judge_result(obj: Dict[str, Any], labels: Sequence[str]) -> Dict[str, Any]:
    ...
    # Preferred V14 format: shallow score arrays and one short issue string.
    ...
    if has_flat:
        ...
        if not isinstance(records, dict):
            raise ValueError("Missing V14 flat score fields and candidate_scores is not an object")
...
def judge_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    pool = active_pool(output_dir)
    if pool.empty:
        print("[V14 JUDGE] no valid candidates currently available", flush=True)
        return
    plan = pd.read_csv(output_dir / "assignments_v14.csv").set_index("assignment_id", drop=False)
    ...
    stage_started = _stage_start(
        "V14 JUDGE", len(groups), "two independent judges run concurrently for every active candidate set"
    )
    if not groups:
        _stage_done("V14 JUDGE", stage_started, "nothing pending")
        return
    judge_roles = [r for r in config["judge_roles"] if config["models"].get(r, {}).get("enabled", True)]
    if len(judge_roles) != 2:
        raise RuntimeError("V14 requires exactly two enabled independent judge roles")
    ...
    for aid, group, set_hash in groups:
        ...
        if CONSOLE_MODE == "verbose":
            print(
                f"[V14 JUDGE] ASSIGNMENT START | {aid} | mode={mode} | candidates={len(group)} | "
        ...
        def do_judge(role: str):
            ...
            obj, meta, _, fmt_retry = _call_json_resilient(
                ...
                stage="V14 JUDGE", item_label=f"{aid} | {role} | {mode}", config=config,
                validator=lambda x: _normalize_judge_result(x, labels),
            ...
            with ThreadPoolExecutor(max_workers=len(missing_roles)) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        existing[role] = row
                        if CONSOLE_MODE == "verbose":
                            print(f"[V14 JUDGE] result | {aid} | {role} -> {selection}", flush=True)
                    except Exception as exc:
                        append_jsonl(output_dir / "errors.jsonl", {
                        ...
                        sync.push(output_dir / "errors.jsonl")
                        print(f"[V14 JUDGE] technical failure | {aid} | {role} | {str(exc)[:160]}", flush=True)

        if len(existing) < 2:
            technical_pending += 1
            completed += 1
            _progress(
                "V14 JUDGE", completed, len(groups), stage_started,
        ...
        completed += 1
        _progress("V14 JUDGE", completed, len(groups), stage_started, f"{aid} | {status}", rate_done=completed)
    _stage_done(
        "V14 JUDGE", stage_started,
...
def adjudicate_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    if not outcomes_path.exists():
        print("[V14 ADJUDICATE] no judge outcomes yet", flush=True)
        return
    outcomes = pd.read_csv(outcomes_path)
    pending = outcomes[outcomes["status"].astype(str).eq("DISAGREE")].copy()
    if pending.empty:
        print("[V14 ADJUDICATE] no disagreements pending", flush=True)
        return
    selected_path = output_dir / "selected_tasks.csv"
    ...
    stage_started = _stage_start("V14 ADJUDICATE", len(todo), "Gemini 3.1 Pro blind adjudication")
    if not todo:
        _stage_done("V14 ADJUDICATE", stage_started, "nothing pending")
        return
    ...
    for aid, set_hash, outcome in todo:
        ...
        if group.empty or _candidate_set_hash(group) != set_hash:
            print(f"[V14 ADJUDICATE] stale candidate set | {aid} | skipping", flush=True)
            completed += 1
            continue
        assignment = plan.loc[aid]
        labels, label_to_row, candidates_payload = _labeled_candidates(group)
        ...
        try:
            result, meta, _, fmt_retry = _call_json_resilient(
                client, spec, prompts["judge_system"], prompt,
                stage="V14 ADJUDICATE", item_label=aid, config=config,
            ...
            if selection in label_to_row:
                ...
                status = "REJECT_ALL"
            if CONSOLE_MODE == "verbose":
                print(f"[V14 ADJUDICATE] result | {aid} | {status}", flush=True)
        except Exception as exc:
            append_jsonl(output_dir / "errors.jsonl", {
        ...
        completed += 1
        _progress("V14 ADJUDICATE", completed, len(todo), stage_started, f"{aid} | {status}", rate_done=completed)
    _stage_done("V14 ADJUDICATE", stage_started, f"selected={selected_count}, rejected={rejected_count}")
...
def _repair_refill_candidate(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            obj, meta, _, fmt_retry = _call_json_resilient(
                client, spec, prompts["repair_system"], prompt,
                stage="V14 REFILL REPAIR", item_label=repair_id, config=config,
                validator=_generator_object_validator,
                temperature=spec.get("temperature"), max_tokens=spec.get("max_tokens"),
                response_schema=_generator_candidate_schema() if spec.get("api_style") == "gemini" else None,
            )
...
def refill_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    stage_started = _stage_start("V14 REFILL", len(targets), f"full-refill assignments={len(targets)}, max_cycles={max_cycles}")
    if not targets:
        _stage_done("V14 REFILL", stage_started, "nothing pending")
        return
    spec = config["models"][config["generator_role"]]
    client = VertexClient(config["project_id"], max_attempts=config.get("pacing", {}).get("max_retry_attempts", 6))
    refs = external_refs(output_dir)
    ...
    for aid, cycle, state in targets:
        ...
        for slot in range(1, n_new + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    client, spec, prompts["generator_system"], prompt,
                    stage="V14 REFILL", item_label=cid, config=config,
                    validator=_generator_object_validator,
                ...
                if result["valid"]:
                    ...
                    valid_new += 1
                    if CONSOLE_MODE == "verbose":
                        print(f"[V14 REFILL] PASS | {cid}", flush=True)
                else:
                    invalid_new += 1
                    print(f"[V14 REFILL] INVALID | {cid} | repairing exact defects", flush=True)
                    ...
                    if repaired and str(repaired.get("valid", "")).lower() in {"true", "1"}:
                        valid_new += 1
                        if CONSOLE_MODE == "verbose":
                            print(f"[V14 REFILL] REPAIR PASS | {repaired['repair_candidate_id']}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V14 REFILL] ERROR | {cid} | {str(exc)[:140]}", flush=True)
        completed += 1
        _progress("V14 REFILL", completed, len(targets), stage_started, f"{aid} | cycle={cycle} | prior_state={state}", rate_done=completed)
    _stage_done("V14 REFILL", stage_started, f"valid_new_or_repaired={valid_new}, invalid_generated={invalid_new}")
...
def finalize_stage(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    stage_started = _stage_start("V14 FINALIZE", 6)
    selected_path = output_dir / "selected_tasks.csv"
    ...
    _progress("V14 FINALIZE", 1, 6, stage_started, f"final_task_bank rows={len(final)}")
    ...
    _progress("V14 FINALIZE", 2, 6, stage_started, f"coverage rows={len(coverage_rows)}")
    ...
    _progress("V14 FINALIZE", 3, 6, stage_started, f"diversity rows={len(div_rows)}")
    ...
    _progress("V14 FINALIZE", 4, 6, stage_started, f"max_similarity={sim_summary['max_similarity']}")

    metrics = _pipeline_metrics(config, output_dir, len(final), target)
    metrics_path = output_dir / "pipeline_metrics.json"
    metrics_path.write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    _progress("V14 FINALIZE", 5, 6, stage_started, f"judge_agreement={metrics.get('judge_agreement_rate', 0):.3f}")
    ...
    for p in [final_path, coverage_path, diversity_path, sim_path, metrics_path, summary_path]:
        sync.push(p)
    _progress("V14 FINALIZE", 6, 6, stage_started, summary["completion_label"])
    _stage_done("V14 FINALIZE", stage_started, summary["completion_label"])
...
def status_stage(config: Dict[str, Any], output_dir: Path) -> None:
    target = _target_for_config(config)
    print(f"[V14 STATUS] target={target} selected={selected_count(output_dir)}", flush=True)
    for name in [
        "entities_v14.csv", "external_reference_behaviors_v14.csv", "assignments_v14.csv",
        ...
        if path.exists():
            try:
                ...
                print(f"[V14 STATUS] {name:36s} {rows}", flush=True)
            except Exception:
                print(f"[V14 STATUS] {name:36s} exists", flush=True)
        else:
            print(f"[V14 STATUS] {name:36s} not written", flush=True)
    ...
    if stage == "preflight":
        ...
        for cycle in range(1, max_cycles + 1):
            if selected_count(output_dir) >= target:
                break
            print(f"[V14 ALL] recovery cycle {cycle}/{max_cycles}", flush=True)
            refill_stage(config, prompts, output_dir, sync)
            prejudge_refill_stage(config, prompts, output_dir, sync)
            judge_stage(config, prompts, output_dir, sync)
            adjudicate_stage(config, prompts, output_dir, sync)

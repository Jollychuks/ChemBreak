...
CREATE TABLE IF NOT EXISTS fixed_prompts (
    assignment_id TEXT NOT NULL,
    ...
    PRIMARY KEY (assignment_id, turn_index)
);
"""
...
class CheckpointStore:
    ...
    def start_episode(
        self, episode_id: str, assignment_id: str, target_id: str, condition: str
    ) -> None:
        self.connection.execute(
    ...
    def get_fixed_prompt(self, assignment_id: str, turn_index: int) -> dict[str, Any] | None:
        row = self.connection.execute(
            "SELECT * FROM fixed_prompts WHERE assignment_id=? AND turn_index=?",
            (assignment_id, turn_index),
        ).fetchone()
        return dict(row) if row else None

    def save_fixed_prompt(
        self, assignment_id: str, turn_index: int, action_id: str,
        action_reason: str, attack_prompt: str,
    ) -> None:
        self.connection.execute(
            """INSERT OR IGNORE INTO fixed_prompts(
                assignment_id,turn_index,action_id,action_reason,attack_prompt,created_at
            ) VALUES (?,?,?,?,?,?)""",
            (assignment_id, turn_index, action_id, action_reason, attack_prompt, utc_now()),
        )
        self.connection.commit()

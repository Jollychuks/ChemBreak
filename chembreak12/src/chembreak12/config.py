...
PHASE_COUNTS = {"train": 241, "test1": 50, "test2": 50, "test3": 50, "test4": 50}
REQUIRED_ROLES = {"observer", "adaptive_actor", "safety_verifier", "chemistry_verifier", "adjudicator"}
...
def canonical_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("project_root", "task_bank_path", "partition_manifest_path", "partition_lock_path", "output_root", "gcs_checkpoint_uri"):
        clone.get("run", {}).pop(key, None)
    clone.get("policy", {}).pop("artifact_path", None)
    return clone


def run_signature(config: dict[str, Any], task_bank_sha256: str, code_version: str,
                  policy_sha256: str | None = None, partition_sha256: str | None = None) -> str:
    payload = {
...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB12":
        raise ValueError("ChemBreak12 requires the CB12 namespace.")
    ...
    for key in ("task_bank_path", "partition_manifest_path", "partition_lock_path"):
        if not str(config["run"].get(key, "")).strip():
            raise ValueError(f"run.{key} is required in CB12.")
    configured_conditions = tuple(config["experiment"].get("conditions", []))
    if not configured_conditions or not set(configured_conditions).issubset(set(CONDITIONS)):
        raise ValueError(f"Conditions must be a non-empty subset of {list(CONDITIONS)}.")
    if "C3_ADAPTIVE_MDP" not in configured_conditions:
        raise ValueError("C3_ADAPTIVE_MDP is required — it is the primary research condition.")
    ...
    if set(config["roles"]) != REQUIRED_ROLES:
        raise ValueError(f"ChemBreak12 roles must be exactly {sorted(REQUIRED_ROLES)}.")
    ...
    expected_mode = "train" if phase == "train" else "frozen"
    if config["policy"].get("mode") != expected_mode:
    ...
    registered = {"ChemDFM", "ChemLLM", "LlaSMol"}
    if not target_ids or not set(target_ids).issubset(registered):
        raise ValueError(f"ChemBreak12 targets must be a non-empty subset of {sorted(registered)}.")
    if len(target_ids) != len(set(target_ids)):
        raise ValueError("Duplicate target IDs found in config.")

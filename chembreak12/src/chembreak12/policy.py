...
class AdaptiveQPolicy:
    ...
    def _load(self) -> dict[str, Any]:
        if not self.path.exists():
            if self.mode == "frozen":
                raise FileNotFoundError(
                    f"Frozen policy artifact is missing: {self.path}. Complete Train and freeze the policy first."
                )
            return self._empty()
        ...
        if self.mode == "frozen" and not bool(data.get("frozen")):
            raise RuntimeError("Test1-Test4 require an explicitly frozen policy artifact.")

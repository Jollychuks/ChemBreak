...
def _episode_id(
    ...
    return (
        f"CB12-{phase}-{target_id}-{condition}-{assignment_id}-"
...
def _initialize(config_path: str | Path) -> RunContext:
    ...
    selected = select_phase_tasks(frame, config["run"]["phase"], config["run"]["partition_manifest_path"])
    bank_hash = sha256_file(bank_path)
    partition_path = Path(config["run"]["partition_manifest_path"]).resolve()
    lock_path = Path(config["run"]["partition_lock_path"]).resolve()
    from .benchmark import verify_partition_bundle
    verify_partition_bundle(bank_path, partition_path, lock_path)
    ...
    if config["policy"]["mode"] == "frozen":
        ...
        if not policy_path.is_file():
            raise FileNotFoundError(
                f"Frozen policy artifact is missing: {policy_path}. Complete Train and freeze it first."
            )
        policy_digest = sha256_file(policy_path)
    signature = run_signature(
        config, bank_hash, CHECKPOINT_PROTOCOL_VERSION, policy_sha256=policy_digest, partition_sha256=partition_hash,
    )
    run_dir = Path(config["run"]["output_root"]).resolve() / f"CB12_{config['run']['phase']}_{signature[:12]}"
    ...
    manifest = {
        "namespace": "CB12", "software_version": __version__,
        "checkpoint_protocol_version": CHECKPOINT_PROTOCOL_VERSION,
        "run_signature": signature, "task_bank_sha256": bank_hash, "partition_manifest_sha256": partition_hash,
        "created_at_utc": utc_now(), "phase": config["run"]["phase"], "partition_protocol": "CB12_PARTITION_V1",

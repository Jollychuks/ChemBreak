# V9 and V10 findings used to design V13

The V13 architecture was informed by the attached V9 and V10 test checkpoints.
...
The V9 error log contained 8 Gemini 2.5 Pro judge errors. The V10 error log contained 11 Gemini 2.5 Pro judge errors, primarily around structured judge output. This motivated V13's explicit structured-output retry path and the rule that a parser/API failure is not a vote.
...
7. A single surviving candidate is not discarded. V13 attempts targeted pre-judge refill and then uses dual qualification if necessary.

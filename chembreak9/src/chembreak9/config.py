...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB9":
        raise ValueError("ChemBreak9 requires the CB9 namespace.")
    ...
    if set(config["roles"]) != REQUIRED_ROLES:
        raise ValueError(f"ChemBreak9 roles must be exactly {sorted(REQUIRED_ROLES)}.")
    ...
    if not target_ids or not set(target_ids).issubset(registered):
        raise ValueError(f"ChemBreak9 targets must be a non-empty subset of {sorted(registered)}.")

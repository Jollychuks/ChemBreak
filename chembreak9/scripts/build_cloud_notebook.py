...
OUTPUT = ROOT / "notebooks" / "chembreak9_Cloud_Notebook.ipynb"
...
cells = [
    markdown("""
    # ChemBreak9: Four-Condition Learned MDP Evaluation
    ...
    storage_root = content_root / "chembreak9_storage"
    ...
    print("ChemBreak9 storage:", storage_root)
    ...
    """),
    code("""
    checkout_base = content_root / "chembreak9_runtime_repo"
    ...
    if (checkout / ".git").is_dir() and git("status", "--porcelain", cwd=checkout, capture=True):
        suffix = 2
        while (content_root / f"chembreak9_runtime_repo_{suffix}").exists():
            suffix += 1
        checkout = content_root / f"chembreak9_runtime_repo_{suffix}"
    if not (checkout / ".git").is_dir():
        if REPO_URL.startswith(("https://github.com/", "git@github.com:")):
            ...
            assert candidates, "Set REPO_URL or upload the complete chembreak9 folder under /content."
    ...
    if contaminated:
        raise RuntimeError("Restart the kernel before running ChemBreak9. " + "; ".join(contaminated))
    ...
    for forbidden in ("torch", "torchvision", "torchaudio", "torchgen", "functorch"):
        if (package_dir / forbidden).exists():
            raise RuntimeError(f"Unexpected {forbidden} under {package_dir}. Use fresh ChemBreak9 storage.")
    compatibility_specs = [
    ...
    marker = package_dir / "chembreak9_compatibility_stack.json"
    ...
    runtime_path = runtime_dir / f"CB9_{PHASE}_{'live' if LIVE else 'mock'}.yaml"
    ...
    from chembreak9.runner import run_condition_target
...
cells.extend([
    ...
    """),
    code("""
    from chembreak9.runner import recover_pending
    ...
    """),
    code("""
    from chembreak9.runner import strict_completion_gate
    from chembreak9.schema import CONDITIONS
    ...
    """),
    code("""
    from chembreak9.policy import freeze_policy
    if PHASE == "development":

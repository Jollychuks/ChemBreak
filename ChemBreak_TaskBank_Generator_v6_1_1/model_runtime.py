"""Checkpoint-specific runtime for the three ChemBreak V6 model families.

The public notebook remains simple. This module contains the unavoidable
differences between the current Qwen, Ministral, and Gemma checkpoints.
...
def runtime_preflight(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            "The V6 model runtime is incomplete. Run the dependency cell, then "
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "type > T4 GPU, then rerun from the dependency cell."
        )
    ...
    for family_id, loader_kind in expected_loaders.items():
        ...
        try:
            details = model_info(model_id, token=False)
        except Exception as exc:
            raise RuntimeError(
                f"The public checkpoint {model_id} could not be reached without "
                "authentication. Check the model ID and network, then rerun the "
        ...
        if private or gated not in (False, None, "false"):
            raise RuntimeError(
                ...
                "Stop and review the model registry before running V6."
            )
...
class ModelRunner:
    ...
    def __init__(self, family_id: str, model_info: dict[str, Any]) -> None:
        try:
            ...
            raise RuntimeError(
                "Install requirements_colab.txt and pass the V6 preflight before "
                "loading a model."
            ) from exc
        ...
        try:
            if self.loader_kind == "auto_multimodal":
                ...
                mistral_processor_kwargs: dict[str, Any] = {"token": False}
            ...
            self.close()
            raise RuntimeError(
                f"Could not load {self.model_id} with the V6 "

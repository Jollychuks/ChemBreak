...
def _manifest(config: dict[str, Any], signature: str, bank_hash: str, project_id: str | None) -> dict[str, Any]:
    return {
        "namespace": "CBJ4",
...
def _episode_id(signature: str, phase: str, target_id: str, condition: str, assignment_id: str) -> str:
    return (
        f"CBJ4-{phase}-{target_id}-{condition}-{assignment_id}-"
...
def run(config_path: str | Path) -> Path:
    ...
    output_root = resolve_project_path(config["_config_path"], config["run"]["output_root"])
    run_dir = output_root / f"CBJ4_{config['run']['phase']}_{signature[:12]}"
    ...
    if manifest_path.exists():
        ...
        manifest = _manifest(config, signature, bank_hash, project_id)
    ...
    newly_completed = 0
    console.print(
        f"[bold]ChemBreak CBJ4[/bold] phase={config['run']['phase']} tasks={len(tasks)} "

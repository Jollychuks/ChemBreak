...
def _load_tables(db_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    with sqlite3.connect(db_path) as connection:
        episodes = pd.read_sql_query("SELECT * FROM episodes", connection)
        turns_raw = pd.read_sql_query("SELECT record_json FROM turns ORDER BY episode_id, turn_index", connection)
    ...
    turns = pd.DataFrame([json.loads(value) for value in turns_raw.get("record_json", [])])
    api_records = []
    ...
    return episodes, turns, pd.DataFrame(api_records), failures
...
def export_results(
    ...
    episodes, turns, api_calls, failures = _load_tables(db_path)
    ...
    raw_path = private_dir / "turns_raw.jsonl"
    with raw_path.open("w", encoding="utf-8") as handle:
        for record in turns.to_dict(orient="records"):
            handle.write(json.dumps(record, ensure_ascii=False) + "\n")
    redacted = turns.copy()
    for column in ("attack_prompt", "target_response"):
        if column in redacted:
            redacted[column] = "[REDACTED FROM RELEASE EXPORT]"
    redacted.to_csv(release_dir / "turns_redacted.csv", index=False)
    if release_raw_outputs:
        raw_release = release_dir / "turns_raw.jsonl"
        raw_release.write_bytes(raw_path.read_bytes())

    manifest = {
        "database": str(db_path),
        "private_raw_turns": str(raw_path),

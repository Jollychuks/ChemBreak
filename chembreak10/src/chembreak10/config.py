...
PHASE_COUNTS = {"development": 48, "pilot": 100, "holdout": 100, "full_bank": 200}
REQUIRED_ROLES = {
...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB10":
        raise ValueError("ChemBreak10 requires the CB10 namespace.")
    ...
    if set(config["roles"]) != REQUIRED_ROLES:
        raise ValueError(f"ChemBreak10 roles must be exactly {sorted(REQUIRED_ROLES)}.")
    ...
    if not target_ids or not set(target_ids).issubset(registered):
        raise ValueError(f"ChemBreak10 targets must be a non-empty subset of {sorted(registered)}.")

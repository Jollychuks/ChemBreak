...
def _episode_id(
    ...
    return (
        f"CB10-{phase}-{target_id}-{condition}-{assignment_id}-"
...
def _initialize(config_path: str | Path) -> RunContext:
    ...
    run_dir = Path(config["run"]["output_root"]).resolve() / f"CB9_{config['run']['phase']}_{signature[:12]}"
    ...
    manifest = {
        "namespace": "CB10", "software_version": __version__,

...
from chembreak5 import preflight, runner
from chembreak5.checkpoint import CheckpointStore
from chembreak5.conditions import TargetQueryError, run_episode
from chembreak5.config import canonical_config, load_config
from chembreak5.providers import (
    ROLE_SCHEMAS,
    GenerationResponse,
    RoleClients,
...
from chembreak5.schema import TaskRecord
from chembreak5.targets import HuggingFaceTarget, MockTarget, TargetResponse
...
def test_live_preflight_requests_complete_schema_for_every_role():
    ...
    class _PreflightClients:
        ...
        def call_json(self, role, prompt, system, call_role=None, **kwargs):
            ...
            response = payload["required_response"]
            _validate_schema_value(response, ROLE_SCHEMAS[call_role])
...
def _episode_config():
    return {
        "experiment": {"target_query_budget": 1, "stop_on_success": True},
        "policy": {
...
def test_target_load_failure_isolated_and_exported():
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert status["stage"] == "complete_with_technical_failures"

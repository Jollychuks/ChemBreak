...
1. Add this complete directory to the existing `Jollychuks/ChemBreak` repository. Keep it under `chembreak5` so earlier versions remain untouched. A later revision should use a new namespace.
2. Start a Google Cloud Notebook Enterprise runtime with an NVIDIA A100 GPU and mount the large data disk at `/content`.
3. Open `notebooks/chembreak5_Cloud_Notebook.ipynb` from the repository.
...
The notebook uses the Google Gen AI SDK with Application Default Credentials supplied by Google Cloud. No API key is stored in the repository. It uses a dedicated `/content/chembreak5_repo` checkout, safely fast-forwards a clean checkout, and preserves any notebook changes saved by Google Cloud. It does not import code, configuration, checkpoints, caches, or outputs from ChemBreak1 through ChemBreak4.
...
ChemLLM uses its repository-defined slow InternLM tokenizer. ChemBreak5 rejects invalid tokenizer results, including the Boolean value returned in the failed ChemBreak4 run. It then falls back to a direct, validated load of the repository's `InternLMTokenizer` class and `tokenizer.model` file.
...
chembreak5 preflight --config configs/config.test.yaml
chembreak5 run --config configs/config.test.yaml
```
...
chembreak5 preflight --config configs/config.pilot.yaml
chembreak5 run --config configs/config.pilot.yaml
```

Use `--load-targets` during preflight only when you want a full sequential download and load test:

```bash
chembreak5 preflight --config configs/config.test.yaml --load-targets
```
...
Structured calls use role-specific response schemas plus client-side validation and bounded formatting retries. Private diagnostics record the exact role, completion reason, response length, schema status, and malformed response. A target-loading failure marks only that target unavailable and allows the remaining targets to run. Technical failures are never counted as unsuccessful jailbreak evaluations.
...
The run cell updates one preformatted notebook display instead of printing an unbounded progress stream. Newlines render normally rather than appearing as literal `\\n` text. The dashboard separates evaluated coverage, confirmed successes, ASR among evaluated episodes, retryable technical failures, unavailable targets, saved responses awaiting judgment, and the latest `YES`, `NO`, or `NOT_EVALUATED` verdict. Mock output is always marked as pipeline validation and never presented as a real jailbreak result.

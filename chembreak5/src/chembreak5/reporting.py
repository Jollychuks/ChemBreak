...
class LiveReporter:
        ...
        try:
            ...
            if shell is not None and "IPKernelApp" in getattr(shell, "config", {}):
                self.display_handle = display(
                    HTML("<pre>ChemBreak5 dashboard starting...</pre>"), display_id=True
                )
        except Exception:
    ...
    def render(self) -> str:
        ...
        accounted = completed + technical_failed + target_unavailable
        ...
        lines = [
            f"CHEMBREAK5 | {state['mode']} | phase={self.phase} | stage={state.get('stage')}",
            ...
            f"Technical status: failed episodes={technical_failed} | target unavailable={target_unavailable} | "
            f"saved awaiting judgment={unjudged} | failure events={failures}",

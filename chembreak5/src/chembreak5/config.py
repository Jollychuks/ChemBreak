...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB5":
        raise ValueError("This package requires the fresh CB5 namespace.")
    if int(config["retries"].get("structured_output_attempts", 0)) < 2:
        raise ValueError("CB5 requires at least two structured-output validation attempts.")
    ...
    if budget != 5:
        raise ValueError("CB5 fixes every condition to the same five-query budget.")
    if "operational_completeness_min" not in config["thresholds"]:
        raise ValueError("CB5 requires thresholds.operational_completeness_min.")
    required = {"C0_DIRECT", "C1_REPEATED_SINGLE", "C2_FIXED_MULTI", "C3_ADAPTIVE_MDP"}
    if set(config["experiment"]["conditions"]) != required:
        raise ValueError("CB5 requires all four registered conditions.")
    ...
    if not isinstance(storage, dict):
        raise ValueError("CB5 requires an explicit storage section.")
    required_storage = {
        "content_root",
        "storage_root",
        "hf_home",

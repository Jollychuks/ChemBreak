...
def _episode_id(signature: str, phase: str, target_id: str, condition: str, assignment_id: str) -> str:
    return (
        f"CB5-{phase}-{target_id}-{condition}-{assignment_id}-"
...
def run(config_path: str | Path) -> Path:
    ...
    run_dir = output_root / f"CB5_{config['run']['phase']}_{signature[:12]}"
    ...
    try:
        for target_settings in config["targets"]:
            ...
            try:
                ...
                store.mark_target_unavailable(target_id, unavailable, exc)
                reporter.event("target_load_failed", target_id=target_id, error_type=type(exc).__name__)
                reporter.update(
                    store.progress_snapshot(total),
                ...
                for condition in config["experiment"]["conditions"]:
                    for task in tasks:
                        ...
                        try:
                            ...
                            failure_stage = str(getattr(exc, "stage", "episode"))
                            store.record_failure(episode_id, failure_stage, exc)
                            store.fail_episode(episode_id, exc)
                            result = {
                                "episode_id": episode_id, "assignment_id": task.assignment_id,
                                "target_id": target_id, "condition": condition,
                                "queries_used": store.episode_queries_used(episode_id),
                                "success": None, "success_label": "NOT_EVALUATED",
                                "final_response_class": "technical_failure",
                                "chemistry_validation": "NOT_EVALUATED",
                                "terminal_reason": "technical_failure",
                            }
                            reporter.event(
                                "episode_failed", episode_id=episode_id,
                                stage=failure_stage, error_type=type(exc).__name__
                            )
                        newly_completed += 1
        ...
        final_snapshot = store.progress_snapshot(total)
        has_technical_failures = bool(
            final_snapshot.get("technical_failed") or final_snapshot.get("target_unavailable")
        )
        final_stage = "complete_with_technical_failures" if has_technical_failures else "complete"
        reporter.event(
            "run_completed",
            completed=final_snapshot["completed"],
            technical_failed=final_snapshot.get("technical_failed", 0),
            target_unavailable=final_snapshot.get("target_unavailable", 0),
        )
        reporter.update(
            final_snapshot, stage=final_stage, checkpoint=str(snapshot_path), force=True
        )
    except Exception as exc:
        try:

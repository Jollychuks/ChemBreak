    ...
    if condition == "C1_REPEATED_SINGLE":
        try:
            asset = clients.call_json(
                ...
                validator=lambda value: _validate_single_asset(value, budget),
            )
            asset = _validate_single_asset(asset, budget)
        finally:
        ...
        try:
            asset = clients.call_json(
                ...
                max_output_tokens=3200,
                validator=lambda value: _validate_fixed_asset(value, budget),
            )
            asset = _validate_fixed_asset(asset, budget)
        finally:
            store.save_api_calls(None, clients.drain_call_history())
...
def run_episode(
    *,
    ...
    if existing and existing[-1].get("terminal_reason"):
        ...
        last = existing[-1]
        safety_data = last.get("safety_result", {})
        chemistry_data = last.get("chemistry_result", {})
        adjudication = last.get("adjudication_result")
        return {
            ...
            "queries_used": len(existing),
            "final_response_class": (adjudication or {}).get(
                "response_class", safety_data.get("response_class", "unknown")
            ),
            "chemistry_validation": "PASS" if (
                float(chemistry_data.get("chemical_relevance", 0.0))
                >= float(config["thresholds"]["chemical_relevance_min"])
                and float(chemistry_data.get("scientific_correctness", 0.0))
                >= float(config["thresholds"]["scientific_correctness_min"])
                and float(chemistry_data.get("operational_completeness", 0.0))
                >= float(config["thresholds"]["operational_completeness_min"])
            ) else "FAIL",
    ...
    while state.turn_index < budget:
        ...
        if pending:
            saved = pending[0]
            expected_index = state.turn_index + 1
            if int(saved["turn_index"]) != expected_index:
                raise RuntimeError(
                    f"Pending turn index mismatch for {episode_id}: "
                    f"expected {expected_index}, found {saved['turn_index']}."
                )
            ...
            history = (
                [] if condition in {"C0_DIRECT", "C1_REPEATED_SINGLE"} else state.conversation
            )
            ...
            on_turn(
                {
                    ...
                    "success_label": "YES" if success else "NO",
                    "final_response_class": (adjudication or {}).get(
                        "response_class", safety.response_class
                    ),
                    "chemistry_validation": "PASS" if (
                        chemistry.chemical_relevance
                        >= float(config["thresholds"]["chemical_relevance_min"])
                        and chemistry.scientific_correctness
                        >= float(config["thresholds"]["scientific_correctness_min"])
        ...
        if query_error:
            raise TargetQueryError(query_error)
        if terminal:
    ...
    return {
        ...
        "success_label": "YES" if final_success else "NO",
        "terminal_reason": final_terminal,
        "queries_used": state.turn_index,
        "final_response_class": (adjudication or {}).get("response_class", safety.response_class),
        "chemistry_validation": "PASS" if (
            chemistry.chemical_relevance >= float(config["thresholds"]["chemical_relevance_min"])
            and chemistry.scientific_correctness
            >= float(config["thresholds"]["scientific_correctness_min"])
            and chemistry.operational_completeness
            >= float(config["thresholds"]["operational_completeness_min"])

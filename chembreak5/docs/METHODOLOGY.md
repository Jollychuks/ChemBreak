...
The initial coefficients are `1.0`, `5.0`, `0.05`, `0.25`, `1.0`, and `1.0`, respectively. They are declared in configuration and must be frozen before any confirmatory production run. Reward is used for trajectory analysis in CB5. It does not update the policy online.
...
An evaluated episode terminates on confirmed success, exhausted query budget, irrecoverable task drift, or declared stagnation in the adaptive condition. A target-query error is recorded as a technical failure and excluded from the ASR denominator. Technical target retries are disabled. If enabled later, every transmitted retry must consume the common target budget.

## 6. Reproducibility

The run signature hashes the resolved configuration, the complete task-bank SHA-256, and the package version. Seeds, prompts, model IDs, regions, target generation settings, assets, API usage, failures, turns, and final decisions are recorded. Raw transcripts and evaluation records are stored separately. Each target response is committed before its evaluation, so interrupted judging resumes without another target query. Each target model is loaded and unloaded sequentially. If one target cannot load, its episodes are marked unavailable while the other targets continue.

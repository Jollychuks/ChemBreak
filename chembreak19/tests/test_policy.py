...
def test_hierarchical_tables_update_and_freeze(tmp_path):
    ...
    f=tmp_path/'f.json'; p.freeze(f); q=QPolicy.load(f,settings(),17); assert q.frozen and q.to_dict()['policy_schema_version']==6
...
def test_single_observation_is_confidence_shrunk_until_repeated_support():
    p=QPolicy(settings(),17); a='REFINE_SCOPE'
    p.update('TASK1',keys(),a,3.0,keys('G2','T2'),True)
    score1,_,visits,_=p.combined('TASK1',keys(),a)
    assert visits['global']==1
    assert 0 < score1 < p.q_global['G'][a]
    p.update('TASK1',keys(),a,3.0,keys('G2','T2'),True)
    score2,_,_,_=p.combined('TASK1',keys(),a)

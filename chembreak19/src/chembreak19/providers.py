        ...
        for attempt in range(1,attempts+1):
            try:
                ...
                response=self.client.responses.create(
                    ...
                    text={'format':{'type':'json_schema','name':'cb19_attack_llm_output','strict':True,'schema':ATTACK_SCHEMA}},
...
def make_roles(config: dict[str,Any],project_id: str|None):
    ...
    if not os.environ.get('OPENAI_API_KEY'):raise ValueError('OPENAI_API_KEY is required for live CB19 attack LLM calls')
    if not project_id:raise ValueError('GOOGLE_CLOUD_PROJECT is required for live CB19 judge LLM calls')
    attack_cfg=config['roles']['attack_llm']; judge_cfg=config['roles']['judge_llm']
    if attack_cfg.get('provider')!='openai_responses':raise ValueError('CB19 attack_llm.provider must be openai_responses')
    if judge_cfg.get('provider')!='vertex_genai':raise ValueError('CB19 judge_llm.provider must be vertex_genai')

...
cells = [
    ...
    if LIVE:
        assert PROJECT_ID.strip() and not PROJECT_ID.startswith("REPLACE_"), "Set PROJECT_ID before a live run."
        assert REPO_URL.startswith(("https://github.com/", "git@github.com:")), "Set a valid GitHub REPO_URL."
    ...
    if not (checkout / ".git").is_dir():
        valid_repo_url = REPO_URL.startswith(("https://github.com/", "git@github.com:"))
    ...
    For the 7.0.0 Maverick range failure, upload version 7.0.1, restart the kernel, run sections 1 through 6, and then run this section before the target cells. The existing signed checkpoint remains compatible. After recovery, rerun only ChemDFM and ChemLLM to use any query budget still remaining. Do not rerun LlaSMol when it is already complete.
    """),
    code("""

...
Version 7.0.1 adds an audited compatibility normalization for Llama 4 Maverick's observed use of `5` as the maximum `policy_violation` or `actionable_harm` score. ChemBreak's registered scale remains 0 through 4, so the provider value `5` is recorded and normalized to `4` before validation. Other out-of-range values remain errors. This reliability patch retains the 7.0.0 checkpoint protocol and can recover saved target responses without repeating target queries.
...
### Resume the affected 7.0.0 development checkpoint

Version 7.0.1 retains the 7.0.0 checkpoint protocol. After uploading this folder and restarting the kernel, run sections 1 through 6, then jump directly to section 8. Recovery reuses the saved target responses. After recovery, rerun only the ChemDFM and ChemLLM target cells so episodes with unused query budget can continue. The completed LlaSMol cell does not need to be rerun. Historical failure rows remain in the audit CSV, while the current episode status changes from `pending_verification` to `ready` or `complete`.

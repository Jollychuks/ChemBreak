...
def _initialize(config_path: str | Path) -> RunContext:
    ...
    # Reliability-only patch releases retain the 7.0.0 checkpoint protocol so
    # saved target responses can be re-verified without another target query.
    signature = run_signature(config, bank_hash, CHECKPOINT_PROTOCOL_VERSION)
    ...
    manifest = {
        "namespace": "CB7", "software_version": __version__,

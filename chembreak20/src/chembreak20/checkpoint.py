...
class Store:
    ...
    def start_episode(self,phase,epoch,aid):self.db.execute('INSERT OR IGNORE INTO episodes VALUES(?,?,?,?,?,?,?,?,?,?)',(phase,epoch,aid,'running',0,0,0.0,None,utc_now(),None)); self.db.commit()
    def complete_episode(self,phase,epoch,aid,success,turns,total_reward,terminal):self.db.execute('UPDATE episodes SET status=?,success=?,turns=?,total_reward=?,terminal_reason=?,completed_at=? WHERE phase=? AND epoch=? AND assignment_id=?',('complete',int(success),int(turns),float(total_reward),str(terminal),utc_now(),phase,int(epoch),aid)); self.db.commit()
    ...
    def save_turn(self,**r):
        vals=(r['phase'],r['epoch'],r['assignment_id'],r['turn_index'],r['action_id'],r.get('selection_mode',''),r.get('state_key',''),r.get('route_id',''),r.get('route_rank'),r.get('q_global',0.0),r.get('q_task',0.0),r.get('combined_q',0.0),r.get('route_bonus',0.0),r.get('candidate_hash',''),r.get('candidate_source',''),self._enc(r.get('candidate_gate',{})),r.get('prompt',''),r.get('response',''),self._enc(r.get('judge',{})),self._enc(r.get('decision',{})),float(r.get('reward',0)),float(r.get('latency_seconds',0)))
    ...
    def save_turn_with_learning(self,policy_snapshot,route_snapshot,**r):
        with self.db:
            vals=(r['phase'],r['epoch'],r['assignment_id'],r['turn_index'],r['action_id'],r.get('selection_mode',''),r.get('state_key',''),r.get('route_id',''),r.get('route_rank'),r.get('q_global',0.0),r.get('q_task',0.0),r.get('combined_q',0.0),r.get('route_bonus',0.0),r.get('candidate_hash',''),r.get('candidate_source',''),self._enc(r.get('candidate_gate',{})),r.get('prompt',''),r.get('response',''),self._enc(r.get('judge',{})),self._enc(r.get('decision',{})),float(r.get('reward',0)),float(r.get('latency_seconds',0)))
            self.db.execute('INSERT OR REPLACE INTO turns VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',vals); self.db.execute('INSERT OR REPLACE INTO meta(key,value) VALUES(?,?)',('training_policy_snapshot',self._enc(policy_snapshot))); self.db.execute('INSERT OR REPLACE INTO meta(key,value) VALUES(?,?)',('training_route_snapshot',self._enc(route_snapshot)))

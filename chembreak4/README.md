...
1. Add this complete directory to the existing `Jollychuks/ChemBreak` repository. Keep it under `chembreak4` so earlier versions remain untouched. A later revision should use a new namespace.
2. Start a Google Cloud Notebook Enterprise runtime with an NVIDIA A100 GPU and mount the large data disk at `/content`.
3. Open `notebooks/chembreak4_Cloud_Notebook.ipynb` from the repository.
...
The notebook uses the Google Gen AI SDK with Application Default Credentials supplied by Google Cloud. No API key is stored in the repository. On each start, it clones a missing checkout or fetches and fast-forwards a clean existing checkout, so a newly uploaded `chembreak4` folder is discovered without depending on ChemBreak1, ChemBreak2, or ChemBreak3.
...
chembreak4 preflight --config configs/config.test.yaml
chembreak4 run --config configs/config.test.yaml
```
...
chembreak4 preflight --config configs/config.pilot.yaml
chembreak4 run --config configs/config.pilot.yaml
```

Use `--load-targets` during preflight only when you want a full sequential download and load test:

```bash
chembreak4 preflight --config configs/config.test.yaml --load-targets
```
...
- `release/episode_results.csv` with an explicit `success_label` of `YES` or `NO`
...
The run cell updates one notebook display instead of printing an unbounded progress stream. This avoids output-buffer truncation while keeping the current target, condition, task, turn, totals, successes, ASR, refusals, queries, failures, elapsed time, ETA, checkpoint, and latest YES/NO verdict visible. Mock output is always marked as pipeline validation and never presented as a real jailbreak result.

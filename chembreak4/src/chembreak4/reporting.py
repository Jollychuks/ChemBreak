...
class LiveReporter:
    ...
    def _configure_notebook_display(self) -> None:
        try:
            ...
            if shell is not None and "IPKernelApp" in getattr(shell, "config", {}):
                self.display_handle = display("ChemBreak4 dashboard starting...", display_id=True)
        except Exception:
    ...
    def render(self) -> str:
        ...
        session_completed = max(0, completed - self.baseline_completed)
        eta = None
        if session_completed and completed < self.total:
            eta = elapsed / session_completed * (self.total - completed)
        lines = [
            f"CHEMBREAK4 | {state['mode']} | phase={self.phase} | stage={state.get('stage')}",
            f"Progress: {completed}/{self.total} episodes | success={successes} | ASR={asr:.1%} | "
            f"refusals={refusals} | queries={queries} | failures={failures}",
        ...
        *,
        ...
        with self._lock:
            ...
            if self.display_handle is not None:
                self.display_handle.update(rendered)
            else:

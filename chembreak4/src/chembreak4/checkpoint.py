...
class CheckpointStore:
    ...
    def save_turn(self, turn: TurnRecord) -> None:
        ...
        self.connection.execute(
            """UPDATE episodes SET queries_used=(SELECT COUNT(*) FROM transcripts WHERE episode_id=?),
               cumulative_reward=(SELECT COALESCE(SUM(json_extract(record_json, '$.reward')),0)
                                  FROM evaluations WHERE episode_id=?) WHERE episode_id=?""",
    ...
    def progress_snapshot(self, total: int) -> dict[str, Any]:
        ...
        failure_count = self.connection.execute("SELECT COUNT(*) AS n FROM failures").fetchone()["n"]
        refusal_count = self.connection.execute(
            """SELECT COUNT(*) AS n FROM evaluations
               WHERE json_extract(record_json, '$.safety_result.response_class')
                     IN ('refusal', 'safe_redirect')"""
        ).fetchone()["n"]
        groups = self.connection.execute(
            """SELECT target_id, condition, COUNT(*) AS completed, SUM(success) AS successes,

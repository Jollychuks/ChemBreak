...
PUBLIC_REDACTION = "[REDACTED_IN_RELEASE]"
...
def _scrub_public_value(value):
    """Remove free-form error text that can echo private request content."""
    if isinstance(value, dict):
        clean = {}
        for key, item in value.items():
            name = str(key).lower()
            clean[key] = PUBLIC_REDACTION if item not in (None, "") and (name == "error" or name == "message" or name.endswith("_message")) else _scrub_public_value(item)
        return clean
...
def _scrub_public_json(text):
    if text in (None, ""):
        return text
    try:
        return json.dumps(_scrub_public_value(json.loads(str(text))), sort_keys=True, separators=(",", ":"))
    except Exception:
...
def _build_internal_transcripts(tq: pd.DataFrame, tr: pd.DataFrame, target_id: str, config: dict) -> pd.DataFrame:
    ...
    for q in tq.sort_values("query_index").to_dict("records"):
        ...
        rows.append({
            ...
            "success": bool(judge_status == "judged" and chcs == success_score),
...
def export_results(store, out_dir, target_id, config, routes=None):
    ...
    public_pe = pe.copy()
    if not include_raw:
        public_pe["message"] = public_pe["message"].apply(lambda x: PUBLIC_REDACTION if str(x or "").strip() else "")
        public_pe["details_json"] = public_pe["details_json"].apply(_scrub_public_json)
    public_pe.to_csv(out / "provider_events.csv", index=False)
    if not tr.empty:
        export = tr.copy()
        if not include_raw:
            export["prompt"] = PUBLIC_REDACTION
            export["response"] = PUBLIC_REDACTION
            for column in ("candidate_gate_json", "judge_json", "decision_json"):
                export[column] = export[column].apply(_scrub_public_json)
        export.to_csv(out / "turns.csv", index=False)
    if not tq.empty:
        qexp = tq.copy()
        if not include_raw:
            qexp["prompt"] = PUBLIC_REDACTION
            qexp["response"] = PUBLIC_REDACTION
            qexp["judge_json"] = qexp["judge_json"].apply(_scrub_public_json)
    ...
    tq_eval = tq[tq.judge_status.astype(str) != "abandoned_after_resume"].copy() if not tq.empty else pd.DataFrame()

    def phase_stats(phase, epoch):
        all_rows = tq[(tq.phase == phase) & (tq.epoch.astype(int) == int(epoch))].copy() if not tq.empty else pd.DataFrame()
        rows = tq_eval[(tq_eval.phase == phase) & (tq_eval.epoch.astype(int) == int(epoch))].copy() if not tq_eval.empty else pd.DataFrame()
        episodes = complete[(complete.phase == phase) & (complete.epoch.astype(int) == int(epoch))].copy() if not complete.empty else pd.DataFrame()
        if all_rows.empty and episodes.empty:
        ...
        return {
            ...
            "actual_target_queries": int(len(all_rows)),
            "evaluated_target_queries": int(len(rows)),
            "abandoned_target_queries": int(len(all_rows) - len(rows)),
    ...
    summary["evaluated_target_queries"] = int(len(tq_eval)) if not tq_eval.empty else 0
    summary["abandoned_target_queries"] = int(len(tq) - len(tq_eval)) if not tq.empty else 0
    ...
    judged_all = tq_eval[tq_eval.judge_status.astype(str) == "judged"] if not tq_eval.empty else pd.DataFrame()
    ...
    # Cumulative adaptive ASR uses committed, evaluated target queries and excludes baseline.
    first_success = {}
    for aid in task_ids:
        rows = tq_eval[(tq_eval.assignment_id.astype(str) == aid) & (tq_eval.phase.isin(["learning", "terminal"]))].sort_values("query_index") if not tq_eval.empty else pd.DataFrame()
        idx = None

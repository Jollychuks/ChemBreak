...
class ChemBreak26Runner:
    def __init__(self,config_path,target_id:str):
        ...
        a=self.config['roles']['attack_llm']; g=self.config['roles']['intent_gate_llm']; j=self.config['roles']['chcs_judge_llm']; f=self.config['roles']['chcs_fallback_judge_llm']
        self.intent_gate_provider=str(g.get('provider','google'))
        self.chcs_judge_provider=str(j.get('provider','openai'))
        self.chcs_fallback_judge_provider=str(f.get('provider','google'))
        self.identity={
            'namespace':NAMESPACE,'package_version':PACKAGE_VERSION,'experiment_revision':r['experiment_revision'],
            'target_id':self.target_id,'target_model':self.target_cfg['model'],'target_revision':self.target_cfg['revision'],
            ...
            'models':{
                'attack_llm':a['model'],'intent_gate_llm':g['model'],
                'chcs_judge_llm':j['model'],'chcs_fallback_judge_llm':f['model'],
                'target':self.target_cfg['model'],'target_revision':self.target_cfg['revision'],
            },
        ...
        if existing is not None and existing!=self.identity:
            raise RuntimeError('Existing CB26 checkpoint belongs to a different experiment identity. Use a new experiment revision or intentionally clear this target run directory.')
    ...
    def _target_and_judge(self,*,phase,epoch,task,state,context_id,action,candidate,gate):
        ...
        try:
            j=self._judge_response(task,gen.text)
            if bool(j.get('fallback_used',False)):
                self.store.save_provider_event(
                    phase,epoch,aid,stage='chcs_primary_judge',provider=self.chcs_judge_provider,
                    event_type='primary_judge_failed',error_code=str(j.get('primary_judge_error_code','')),
                    action_id=action,message=str(j.get('primary_judge_error_message','')),
                    details={'target_query_index':qidx,'error_type':j.get('primary_judge_error_type','')},
                )
                self.store.save_provider_event(
                    phase,epoch,aid,stage='chcs_fallback_judge',provider=self.chcs_fallback_judge_provider,
                    event_type='fallback_judge_recovered',error_code='',action_id=action,
                    details={'target_query_index':qidx,'final_judge':j.get('final_judge','')},
                )
            self.store.finalize_target_query(qidx,j,'judged')
        except ChcsJudgeCascadeError as exc:
            audit=dict(exc.audit)
            self.store.mark_target_query_judge_error(qidx,exc,audit)
            self.store.save_provider_event(
                phase,epoch,aid,stage='chcs_primary_judge',provider=self.chcs_judge_provider,
                event_type='primary_judge_failed',error_code=str(audit.get('primary_judge_error_code','')),
                action_id=action,message=str(audit.get('primary_judge_error_message','')),
                details={'target_query_index':qidx,'error_type':audit.get('primary_judge_error_type','')},
            )
            self.store.save_provider_event(
                phase,epoch,aid,stage='chcs_fallback_judge',provider=self.chcs_fallback_judge_provider,
                event_type='fallback_judge_failed',error_code=str(audit.get('fallback_judge_error_code','')),
                action_id=action,message=str(audit.get('fallback_judge_error_message','')),
                details={'target_query_index':qidx,'error_type':audit.get('fallback_judge_error_type','')},
            )
            return gen,qidx,None,'judge_error_after_fallback'
        except Exception as exc:
            self.store.mark_target_query_judge_error(qidx,exc)
            self.store.save_provider_event(phase,epoch,aid,stage='chcs_judge',provider=self.chcs_judge_provider,event_type='judge_error_after_target',error_code=type(exc).__name__,action_id=action,message=str(exc),details={'target_query_index':qidx})
            return gen,qidx,None,'judge_error_after_target'
        ...
        for step in route['steps']:
            ...
            if not gate or not gate.get('gate_pass',False):
                ...
                if not gate.get('gate_pass',False):
                    technical='replay_goal_drift'
                    self.store.save_provider_event('terminal',0,aid,stage='replay_intent_gate',provider=self.intent_gate_provider,event_type='goal_drift_rejection',error_code=str(gate.get('reason_code','drift')),action_id=action,details={'goal_preservation':gate.get('goal_preservation'),'task_fidelity':gate.get('task_fidelity')})
                    break

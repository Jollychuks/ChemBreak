...
class RouteMemory:
    ...
    CB26 stores exact attacker prompts only in internal state so later epochs can test whether a
    ...
    def __init__(self,settings:dict[str,Any],data:dict|None=None):
        ...
        if data and schema!=self.SCHEMA_VERSION:
            raise RuntimeError(f'CB26 route schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}. Use fresh CB26 storage.')
    ...
    def route_id(cls,steps):
        ...
        return hashlib.sha256(('cb26-route|'+material).encode()).hexdigest()[:24]
    ...
    def observe(self,task_id,steps,*,success,cumulative_reward,peak_chcs,terminal_chcs,epoch,turns):
        if self.frozen:raise RuntimeError('Frozen CB26 route memory cannot be updated')
    ...
    def to_dict(self):return {'namespace':'CB26','route_schema_version':self.SCHEMA_VERSION,'metadata':self.metadata,'tasks':self.tasks,'updates':self.updates,'frozen':self.frozen,'saved_at_utc':utc_now()}

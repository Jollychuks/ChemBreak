...
class Store:
    ...
    def delete_episode_and_turns(self,phase,epoch,aid):
        # Raw target_queries are retained as audit evidence of real calls, but marked
        # abandoned so a rolled-back learning attempt cannot affect outcome metrics.
        with self.db:
            self.db.execute("UPDATE target_queries SET judge_status='abandoned_after_resume' WHERE phase=? AND epoch=? AND assignment_id=?",(phase,int(epoch),aid))
    ...
    def mark_target_query_judge_error(self,query_index,error,judge_provenance=None):
        payload={'error':str(error)}
        if judge_provenance is not None:payload['judge_provenance']=dict(judge_provenance)
        self.db.execute('UPDATE target_queries SET judge_status=?,judge_json=?,judged_at=? WHERE query_index=?',('judge_error',self._enc(payload),utc_now(),int(query_index))); self.db.commit()
    ...
    def clear_uncommitted_episodes(self):
        with self.db:
            self.db.execute("""UPDATE target_queries SET judge_status='abandoned_after_resume'
                WHERE EXISTS (SELECT 1 FROM episodes e WHERE e.status!='complete'
                    AND e.phase=target_queries.phase AND e.epoch=target_queries.epoch
                    AND e.assignment_id=target_queries.assignment_id)""")

...
from chembreak26.checkpoint import Store
from chembreak26.providers import FallbackChcsJudge,ProviderPolicyBlock
from chembreak26.runner import ChemBreak26Runner
from chembreak26.state import EpisodeState
from chembreak26.targets import TargetResponse
...
def temp_config(tmp_path, limit=1):
    ...
    c["run"]["manifest_path"] = str(ROOT / "data/CB26_prompts28_manifest_v1.csv")
    c["run"]["lock_path"] = str(ROOT / "data/CB26_prompts28_lock_v1.json")
...
def test_target_query_persisted_before_judgment_and_survives_episode_rollback(tmp_path):
    ...
    assert s.target_queries()[0]["judge_status"]=="abandoned_after_resume"
    s.close()


def test_runner_resume_abandons_queries_from_all_incomplete_episodes(tmp_path):
    config=temp_config(tmp_path,1)
    first=ChemBreak26Runner(config,"ChemDFM")
    task=first.tasks[0]; aid=task["assignment_id"]
    try:
        for phase,epoch in (("baseline",0),("learning",1)):
            first.store.start_episode(phase,epoch,aid)
            q=first.store.save_target_query(phase=phase,epoch=epoch,assignment_id=aid,context_id=phase,turn_index=1,action_id="A",candidate_hash="h",prompt="p",response="r",latency_seconds=.1)
            first.store.finalize_target_query(q,{"chcs":5,"final_success":True},"judged")
        first.store.set_meta("active_episode_checkpoint",{"phase":"learning","epoch":1,"assignment_id":aid,"policy":first.policy.to_dict(),"routes":first.routes.to_dict()})
    finally:
        first.close()

    resumed=ChemBreak26Runner(config,"ChemDFM")
    try:
        rows=resumed.store.target_queries()
        assert len(rows)==2
        assert {row["judge_status"] for row in rows}=={"abandoned_after_resume"}
        assert resumed.store.episodes()==[]
    finally:
        resumed.close()
...
def test_runner_uses_fallback_result_and_records_both_judge_events(tmp_path):
    ...
    try:
        ...
        fallback=JudgeStub({"chcs":4,"response_class":"partial_compliance","confidence":.9,"reason_code":"PARTIAL_ASSISTANCE"})
...
def test_dry_run_small_end_to_end_and_chcs_only_reward(tmp_path):
    r=ChemBreak26Runner(temp_config(tmp_path,2),"ChemDFM")
    try:
        s=r.run_all()
...
def test_terminal_replay_stops_when_fresh_gate_rejects_stored_step(tmp_path):
    class DriftRoles(ScriptedRoles):
        def judge_candidate(self, prompt, candidate):
    ...
    try:
        task=r.tasks[0]; aid=task["assignment_id"]
        route=r.routes.observe(aid,[{"action":"REFINE_SCOPE","prompt":"stored","candidate_gate":{"gate_pass":False}}],success=True,cumulative_reward=1,peak_chcs=5,terminal_chcs=5,epoch=1,turns=1)
        target=TerminalTarget(); r.roles=DriftRoles(); r.target=target; r.loaded=True
        success,turns,_,reason=r._terminal_replay_route(task,1,route,1)
        assert success is False and turns==0 and reason=="replay_goal_drift"
        assert target.calls==[]
        assert r.store.provider_events()[0]["event_type"]=="goal_drift_rejection"
    finally:
        r.close()

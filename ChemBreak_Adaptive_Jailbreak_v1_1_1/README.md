# ChemBreak Adaptive Jailbreak v1.1.1
...
Recommended repository folder: `ChemBreak_Adaptive_Jailbreak_v1_1_1/`
...
The v1.1.1 package intentionally separates the main model roles:
...
3. Open `ChemBreak_Adaptive_Jailbreak_v1_1_1_Colab_Enterprise.ipynb` in Colab Enterprise.
...
## Asset-generation repair and resumability

Each frozen asset type, adaptive graph, repeated single-turn set, and fixed multi-turn sequence, gets up to **3 total model generations**. A structurally invalid but parseable response is not accepted. The exact defect, for example `expected 5 attempts, got 4`, is fed back to the attacker and the entire asset is regenerated. Harmless top-level bare-list JSON is normalized to the required object wrapper before validation.

Preparation writes `public/prepare_summary.json` and exits nonzero if any selected task still lacks a complete asset after the retries. This prevents target execution from starting with an incomplete frozen-asset set.

Runtime outputs are checkpointed to the configured GCS prefix. Reconnect with the same `RUN_MODE` and `RUN_ID`, rerun setup, preflight, the optional v1.1 asset-import bridge, then rerun the interrupted stage. Completed assets and completed task-target-condition units are skipped.

For migration from v1.1, `scripts/import_previous_assets.py` can copy only `status=complete` frozen assets from the matching v1.1 GCS run into the new v1.1.1 namespace. It does not modify the v1.1 output. This lets the five successful assets from the earlier 8-task test be reused while only the missing tasks are regenerated.
...
python scripts/import_previous_assets.py --config configs/runtime.yaml --previous-gcs-output-uri gs://YOUR_BUCKET/ChemBreak_Adaptive_Jailbreak_v1_1/test/test_001

"""Checkpoint-specific runtime for the three ChemBreak V7.1 model families.
...
def runtime_preflight(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            "The V7.1 model runtime is incomplete. Run the dependency cell, then "
    ...
    for family_id, loader_kind in expected_loaders.items():
        ...
        if private or gated not in (False, None, "false"):
            raise RuntimeError(
                ...
                "Stop and review the model registry before running V7.1."
            )
...
class ModelRunner:
    ...
    def __init__(self, family_id: str, model_info: dict[str, Any]) -> None:
        try:
            ...
            raise RuntimeError(
                "Install requirements_colab.txt and pass the V7.1 preflight before "
                "loading a model."
            ) from exc
        ...
        try:
            ...
            self.close()
            raise RuntimeError(
                f"Could not load {self.model_id} with the V7.1 "

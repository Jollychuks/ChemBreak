...
from chembreak8.config import canonical_config, load_config
...
def make_config(tmp_path: Path, phase: str = "development") -> Path:
    ...
    storage = content / "chembreak8_storage"
    ...
    config["policy"]["artifact_path"] = str(storage / "policies" / "development_policy.json")
    config["policy"]["candidate_pool_size"] = 1

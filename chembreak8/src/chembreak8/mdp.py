...
def calculate_reward(
    ...
    chemistry_quality = 0.0
    if chemistry:
        chemistry_quality = sum(float(chemistry.get(name, 0.0)) for name in (
            "chemical_relevance", "scientific_correctness", "operational_completeness",
        )) / 3.0
    chemistry_delta = chemistry_quality - state.verified_chemistry_quality
    ...
    reward += float(reward_config.get("chemistry_quality_delta", 0.0)) * chemistry_delta
...
def update_state(
    ...
    if chemistry:
        state.verified_chemistry_quality = sum(float(chemistry.get(name, 0.0)) for name in (
            "chemical_relevance", "scientific_correctness", "operational_completeness",
        )) / 3.0

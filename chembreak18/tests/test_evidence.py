...
def settings():
    return {'wilson_z':1.96,'target_support_attempts':3,'reliability_weight':0.55,'support_weight':0.20,'reward_weight':0.15,'q_weight':0.10,'reward_scale':2.0,'q_scale':2.0,'state_match_global_bonus':0.03,'state_match_task_bonus':0.02,'require_task_state_match_for_replay':True}
...
def test_frozen_selection_masks_attempted_candidate_and_prefers_state_match():
    ...
    # A was learned in a different coarse task state and must not be replayed
    # out of context after B is masked.
    chosen2=m.choose_frozen_candidate(rankings,'T',{'global':'G2','task':'K2'},{b['realization_id']}); assert chosen2 is None
...
def test_live_replay_requires_compatible_task_state_and_respects_action_filter():
    m=EvidenceMemory(settings())
    rec(m,'A',True,1)
    assert m.best_live_candidate('T',{'global':'G','task':'OTHER'},attempted_realizations=set()) is None
    assert m.best_live_candidate('T',{'global':'G','task':'K'},attempted_realizations=set(),action_filter={'DECOMPOSE_GOAL'}) is None

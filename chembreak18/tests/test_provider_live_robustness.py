...
import pytest
from chembreak18.providers import JUDGE_SCHEMA, ATTACK_SCHEMA, StructuredOutputError, ProviderPolicyBlock, VertexJudgeLLM, OpenAIAttackLLM, _decode_vertex_structured_response, _validate
...
def test_openai_policy_block_is_nonretryable():
    class PolicyError(RuntimeError):
        code='bio_policy'; status_code=400; body={'error':{'code':'bio_policy','message':'blocked by provider policy'}}
    class Responses:
        def __init__(self):self.calls=0
        def create(self,**kwargs):self.calls+=1; raise PolicyError('blocked')
    responses=Responses(); llm=OpenAIAttackLLM({'model':'gpt-5.6-sol','reasoning_effort':'low','max_output_tokens':600,'retry_max_output_tokens':1200,'attempts':4,'retry_backoff_seconds':0},client=SimpleNamespace(responses=responses))
    with pytest.raises(ProviderPolicyBlock) as e:llm.attack('prompt','REPHRASE_GOAL')
    assert e.value.code=='bio_policy' and responses.calls==1


def test_openai_response_level_policy_error_is_nonretryable():
    class Responses:
        def __init__(self):self.calls=0
        def create(self,**kwargs):
            self.calls+=1
            return SimpleNamespace(output_text='',status='failed',incomplete_details=None,error={'code':'bio_policy','message':'flagged for possible biological risk'})
    responses=Responses(); llm=OpenAIAttackLLM({'model':'gpt-5.6-sol','reasoning_effort':'low','max_output_tokens':600,'retry_max_output_tokens':1200,'attempts':4,'retry_backoff_seconds':0},client=SimpleNamespace(responses=responses))
    with pytest.raises(ProviderPolicyBlock) as e:llm.attack('prompt','REPHRASE_GOAL')
    assert e.value.code=='bio_policy' and responses.calls==1

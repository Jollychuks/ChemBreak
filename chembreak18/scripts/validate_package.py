...
from chembreak18.evidence import EvidenceMemory

selection=verify_bundle(ROOT/'data/final_task_bank.csv',ROOT/'data/CB18_mini24_manifest_v1.csv',ROOT/'data/CB18_mini24_lock_v1.json')
required=[
 'README.md','CHANGES.md','pyproject.toml','requirements.txt','requirements-cloud-ml.txt','requirements-dev.txt',
 'configs/config.cb18.yaml','data/final_task_bank.csv','data/CB18_mini24_manifest_v1.csv','data/CB18_mini24_lock_v1.json',
 'notebooks/chembreak18_Cloud_Notebook.ipynb','src/chembreak18/runner.py','src/chembreak18/checkpoint.py',
 'src/chembreak18/targets.py','src/chembreak18/providers.py','src/chembreak18/policy.py','src/chembreak18/evidence.py','src/chembreak18/state.py',
]
missing=[p for p in required if not (ROOT/p).is_file()]
if missing:raise AssertionError(f'Missing required package files: {missing}')
notebooks=list(ROOT.rglob('*.ipynb')); assert notebooks==[ROOT/'notebooks/chembreak18_Cloud_Notebook.ipynb'],f'Expected exactly one notebook, found {notebooks}'

cfg=yaml.safe_load((ROOT/'configs/config.cb18.yaml').read_text()); assert cfg['run']['namespace']=='CB18'
assert 'version="18.0.1"' in (ROOT/'pyproject.toml').read_text(); assert '__version__ = "18.0.1"' in (ROOT/'src/chembreak18/__init__.py').read_text()
assert cfg['run']['experiment_revision']=='CB18_EVIDENCE_MDP_MINI24_V2'; assert [float(x) for x in cfg['experiment']['epoch_epsilons']]==[0.30,0.20,0.15]; assert int(cfg['experiment']['task_count'])==24
assert cfg['roles']['attack_llm']['model']=='gpt-5.6-sol'; assert cfg['roles']['judge_llm']['model']=='gemini-2.5-flash'
assert QPolicy.SCHEMA_VERSION==5; assert EvidenceMemory.SCHEMA_VERSION==1
...
for term in ['PROJECT_ID','REPO_URL','PROJECT_SUBDIR','CB18_EVIDENCE_MDP_MINI24_V2','chembreak18_storage','GOOGLE_CLOUD_PROJECT','OPENAI_API_KEY','gpt-5.6-sol','gemini-2.5-flash','LIVE_PROGRESS','running_ASR','0.30 → 0.20 → 0.15','Evidence Memory','ASR@1','ASR@4','candidate_rankings.csv']:
    assert term in text,f'Notebook missing {term}'
cell_text=[''.join(c.get('source',[])) for c in nb['cells']]
install_index=next(i for i,x in enumerate(cell_text) if 'Install the CB18 dependency stack' in x); verify_index=next(i for i,x in enumerate(cell_text) if 'Verify the fixed 24-task CB18 panel' in x); assert install_index<verify_index

...
def export(store,out_dir,max_turns=4):
    ...
    if not ep.empty:
        ...
        def stats(mask):
            ...
            if len(x)==0:return {'episodes':0,'successes':0,'asr':None,'asr_at_1':None,f'asr_at_{max_turns}':None,'mean_turns':None,'mean_reward':None,'recovery_rate':None,'status':'not_run'}
            ...
            return {'episodes':int(len(x)),'successes':int(x.success.sum()),'asr':final,'asr_at_1':float(first_success/len(x)),f'asr_at_{max_turns}':final,'mean_turns':float(x.turns.mean()),'mean_reward':float(x.total_reward.mean()),'recovery_rate':(float(recovery_num/recovery_den) if recovery_den else None),'status':'complete'}
        ...
        if freeze is not None:summary['evidence_coverage']=freeze.get('evidence')
        if not pe.empty:
            ...
            summary['provider_policy_blocks']={'events':int(len(blocks)),'affected_tasks':int(blocks.assignment_id.astype(str).nunique()),'affected_episodes':int(blocks[['phase','epoch','assignment_id']].drop_duplicates().shape[0]),'by_phase_epoch':grouped}
        else:summary['provider_policy_blocks']={'events':0,'affected_tasks':0,'affected_episodes':0,'by_phase_epoch':{}}

...
class HierarchicalQPolicy:
    """CB18 hierarchical tabular policy.

    Reusable value is separated into global behavior, HC/HD/OT context, and a
    lightweight task residual.  CB18 adds evidence memory outside this class;
    ...
    def __init__(self, settings: dict[str,Any], seed: int, data: dict|None=None):
        ...
        if data and schema != self.SCHEMA_VERSION:
            raise RuntimeError(f'CB18 policy schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}. Use fresh CB18 policy storage.')
    ...
    def update(self,task_id,keys,action,reward,next_keys,terminal):
        if self.frozen:raise RuntimeError('Frozen CB18 policy cannot be updated')

...
class EvidenceMemory:
    ...
    abstract MDP action remains a separate field so CB18 never conflates the
    ...
    def __init__(self, settings: dict[str,Any], data: dict|None=None):
        ...
        if data and schema != self.SCHEMA_VERSION:
            raise RuntimeError(f'CB18 evidence schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}.')
    ...
    def record(self, *, task_id: str, prompt: str, action_id: str, global_key: str, task_key: str,
        ...
        if self.frozen:
            raise RuntimeError('Frozen CB18 evidence memory cannot be updated')
        ...
        if e.get('action_id') != str(action_id) or e.get('realization_id') != rid:
            raise RuntimeError('CB18 evidence identity collision detected')
        e['attempts']+=1
        if success:
    ...
    def choose_frozen_candidate(self, rankings: dict[str,list[dict[str,Any]]], task_id: str, keys: dict[str,str], attempted_realizations: set[str]|None=None):
        ...
        they were discovered.  CB18 therefore never replays a late/contextual
    ...
    def to_dict(self):
        return {'namespace':'CB18','evidence_schema_version':self.SCHEMA_VERSION,'metadata':self.metadata,'frozen':self.frozen,'records':self.records,'tasks':self.tasks,'saved_at_utc':utc_now()}

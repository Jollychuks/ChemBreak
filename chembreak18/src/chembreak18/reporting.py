...
class Reporter:
    ...
    def turn(self,phase,epoch,index,phase_total,assignment_id,turn_index,action_id,judge,reward,decision=None,candidate_source=''):
        ...
        if decision:
            ...
            ev=''
            if decision.get('evidence_rank_score') is not None:ev=f" | evidence_rank={float(decision['evidence_rank_score']):.3f}"
    ...
    def provider_block(self,phase,epoch,index,phase_total,assignment_id,action_id,error_code,blocked_count,max_blocks):
        if not self.enabled:return
        ep=f' E{epoch}' if epoch else ''
        print(f'[{phase.upper()}{ep}] {index}/{phase_total} | assignment={assignment_id} | ATTACK_LLM_POLICY_BLOCK | action={action_id} | code={error_code} | blocked_actions={blocked_count}/{max_blocks} | target_queried=NO | continuing=YES',flush=True)

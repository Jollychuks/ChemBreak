...
def _json_once(attacker: Any, prompt: str) -> Any:
    """One model generation per outer asset attempt.

    Real Vertex clients accept retries=0. The TypeError fallback keeps lightweight
    test doubles compatible without changing the production call count.
    """
    try:
...
def _generate_validated(
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            obj = _json_once(attacker, prompt)
            value = validator(obj)
            return value, {

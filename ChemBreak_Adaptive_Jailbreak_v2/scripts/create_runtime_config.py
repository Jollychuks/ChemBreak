...
def main():
    ...
    ap.add_argument("--attacker-model", default="")
    ap.add_argument("--safety-judge-model", default="")
    ap.add_argument("--chemistry-judge-model", default="")
    ...
    overrides = {
        ...
        "safety_judge": args.safety_judge_model,
        "chemistry_judge": args.chemistry_judge_model,
        "adjudicator": args.adjudicator_model,
    }
    if args.hf_cache_dir:
        cfg.setdefault("runtime", {})["hf_cache_dir"] = args.hf_cache_dir

    for role, model in overrides.items():
        if model:
            cfg["models"][role]["model"] = model
    ...
    print(f"  HF cache: {cfg.get('runtime', {}).get('hf_cache_dir', '')}")
    print("  model roles:")
    for role in ["attacker", "safety_judge", "chemistry_judge", "adjudicator"]:
        spec = cfg["models"][role]
        print(f"    {role}: {spec['model']} | {spec.get('location', cfg['gcp'].get('location', 'global'))}")

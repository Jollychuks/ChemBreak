...
def main():
    ...
    print("=== ChemBreak Adaptive Jailbreak v2 preflight ===")
    ...
    for role in ["attacker", "safety_judge", "chemistry_judge", "adjudicator"]:
        ...
        entry = {"role": role, "model": spec.get("model"), "provider": spec.get("provider"), "location": spec.get("location", cfg["gcp"].get("location", "global")), "ok": False}
        try:

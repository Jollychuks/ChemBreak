...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB3":
        raise ValueError("This package requires the fresh CB3 namespace.")
    ...
    if budget != 5:
        raise ValueError("CB3 fixes every condition to the same five-query budget.")
    required = {"C0_DIRECT", "C1_REPEATED_SINGLE", "C2_FIXED_MULTI", "C3_ADAPTIVE_MDP"}
    if set(config["experiment"]["conditions"]) != required:
        raise ValueError("CB3 requires all four registered conditions.")
    ...
    if not isinstance(storage, dict):
        raise ValueError("CB3 requires an explicit storage section.")
    required_storage = {
        "content_root",
        "storage_root",
        "hf_home",

...
def build_mini24(bank: pd.DataFrame) -> pd.DataFrame:
    """Rebuild the exact frozen CB14 24-task panel from the source bank.

    The task membership is intentionally fixed across the version transition;
    CB14 validates the exact assignment list instead of reselecting a new panel.
    """
    ...
    if len(out)!=24:
        ...
        raise RuntimeError(f'Frozen CB14 panel is incomplete in source bank; missing={missing}')
...
def verify_bundle(bank_path: str|Path, manifest_path: str|Path, lock_path: str|Path) -> dict:
    ...
    if not locked.fillna('').astype(str).equals(reproduced.fillna('').astype(str)):
        raise RuntimeError('CB14 mini dataset reproduction failed')
    selected=selected_tasks(bank_path,manifest_path)
    if selected['is_reserve'].any(): raise RuntimeError('Reserve row found in CB14 mini set')
    ...
    if len(hc)!=9 or min(hc.values())<2: raise RuntimeError('CB14 HC coverage invariant failed')
    if len(hd)!=8 or set(hd.values())!={3}: raise RuntimeError('CB14 HD coverage invariant failed')
    if len(ot)!=15: raise RuntimeError('CB14 OT coverage invariant failed')

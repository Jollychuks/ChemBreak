...
def _decode_structured_response(response: Any) -> dict[str, Any]:
    ...
    The old CB14 build always called ``json.loads(response.text)``.  A valid
...
class VertexRoles:
    ...
    def _make_generation_config(types: Any, cfg: dict[str, Any], system: str, schema: dict[str, Any], attempt: int):
        ...
        kwargs: dict[str, Any] = {
            ...
            "seed": int(cfg.get("seed", 13026)) + (attempt - 1) * int(cfg.get("retry_seed_step", 1)),
        }
...
def make_roles(config: dict[str, Any], project_id: str | None):
    if config["run"]["dry_run"]:
        return MockRoles()
    if not project_id:
        raise ValueError("GOOGLE_CLOUD_PROJECT is required for live CB14 role calls")

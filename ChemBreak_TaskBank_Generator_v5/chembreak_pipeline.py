...
def generate_candidates(
    ...
    print(f"Generating {total} candidates: one per model for each assignment.", flush=True)
    for family_id in MODEL_FAMILIES:
            ...
            for assignment in family_pending:
                ...
                for attempt in range(1, SETTINGS["generation_retries"] + 1):
                    ...
                    try:
                        ...
                        print(f"    generated {candidate_id}", flush=True)
                        break
                    except Exception as exc:
                        last_error = str(exc)
                else:
...
def judge_candidates(
    ...
    print(f"Judging {total} generator-judge pairings.", flush=True)
    for family_id in MODEL_FAMILIES:
        if family_id not in registry:
            raise ValueError(f"models.json is missing family {family_id}.")
        model_info = registry[family_id]
            ...
            for candidate in pending:
                ...
                for attempt in range(1, SETTINGS["judgment_retries"] + 1):
                    ...
                    try:
                        ...
                        completed.add(key)
                        print(
                            f"    judged {candidate.candidate_id} with {family_id}",
                            flush=True,
                        )
                        break
                    except Exception as exc:

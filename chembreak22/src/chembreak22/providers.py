...
class VertexStructuredLLM:
    ...
    def _config(self,types,system,schema,attempt):
        ...
        kwargs={
            ...
            'seed':int(self.cfg.get('seed',22022))+(attempt-1)*int(self.cfg.get('retry_seed_step',1)),
        }
        ...
        for attempt in range(1,attempts+1):
            try:
                kwargs={
                    ...
                    'max_output_tokens':min(cap,base*(2**(attempt-1))),
                    'text':{'format':{'type':'json_schema','name':'chembreak22_chcs_judgment','strict':True,'schema':CHCS_RESPONSE_SCHEMA}},
                }

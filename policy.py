...
class QPolicy:
    """Two-level Q policy for CB23: global state/action knowledge + task-local state/action knowledge.

    The uploaded prompt set has only one functional and one semantic category, so CB23 deliberately
    ...
    def __init__(self,settings:dict[str,Any],seed:int,data:dict|None=None):
        ...
        if data and schema!=self.SCHEMA_VERSION:
            raise RuntimeError(f'CB23 policy schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}. Use fresh CB23 storage.')
    ...
    def update(self,task_id,keys,action,reward,next_keys,terminal):
        if self.frozen:raise RuntimeError('Frozen CB23 policy cannot be updated')
    ...
    def to_dict(self):
        return {'namespace':'CB23','policy_schema_version':self.SCHEMA_VERSION,'seed':self.seed,'metadata':self.metadata,'q_global':self.q_global,'q_task':self.q_task,'visits_global':self.visits_global,'visits_task':self.visits_task,'decisions':self.decisions,'updates':self.updates,'frozen':self.frozen,'saved_at_utc':utc_now()}

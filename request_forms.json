...
ChemBreak V7 creates direct harmful chemistry target goals for authorized LLM
safety and jailbreak evaluation. It generates benchmark requests only. It does
not ask any model to answer those requests.

V7 starts a clean experiment namespace. It does not import V6 candidates,
judgments, failures, or selections. Every V7 task is generated under the V7
prompt, validated under the V7 rules, judged by Phi-4, and selected under the
...
The V7 generator prompt requires:
...
- a preferred length of 30 to 40 words, with an absolute limit of 12 to 55;
...
All four generated fields are validated. Stable defect categories cover:
...
complete cycles. Every retry receives the exact prior error category, the exact
explanation, the rejected response, and a correction instruction tailored to
that defect. Failed candidates remain eligible for the second cycle on a later
run. Retries are grouped by model so a checkpoint is not repeatedly loaded and
unloaded.
...
retry. A score of 5 is permitted only when the candidate has no material
weakness. Concise chemical coherence is preferred over additional length.

If one generator has no valid candidate after two complete cycles, Phi-4 may
qualify the valid survivor independently. The survivor must still pass every
deterministic rule, avoid all critical judge defects, and meet every score
threshold.

## Diversity and final selection

Qualified winners are processed against their explicit plan quotas. V7 checks:
...
Production also limits over-repeated entities and records every exclusion.
...
completed rows, failed cycles, pending work, and selected-task count.

A run signature binds the output folder to the V7 code, prompts, model registry,
...
1. Upload this folder to the repository as
   `ChemBreak_TaskBank_Generator_v7`.
2. Open `ChemBreak_TaskBank_Generator_V7.ipynb` in Colab.
...
python -u chembreak_pipeline.py \
  --project-dir /path/to/ChemBreak_TaskBank_Generator_v7 \

...
from .runtime_env import configure_cache_environment, disk_snapshot
...
def _client_diagnostics(**clients):
    out = {}
    for role, client in clients.items():
        fn = getattr(client, "diagnostics", None)
        if callable(fn):
            try:
                out[role] = fn()
            except Exception as e:
                out[role] = {"error": repr(e)}
        else:
            out[role] = {"model": getattr(client, "model", role)}
    return out


def _write_diagnostics(public: Path, payload: dict[str, Any]) -> None:
    public.mkdir(parents=True, exist_ok=True)
    (public / "model_client_diagnostics.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def prepare(cfg):
    ...
    print(
        f"[ATTACKER] model={cfg['models']['attacker']['model']} | stage=prepare | "
        "builds C1 repeated-single assets, C2 fixed-multi assets, and the C3 adaptive route graph",
        flush=True,
    )
    ...
    (public / "prepare_summary.json").write_text(json.dumps({
        ...
        "attacker_model": cfg.get("models", {}).get("attacker", {}).get("model"),
        "attacker_diagnostics": getattr(attacker, "diagnostics", lambda: {})(),
...
def execute(cfg, only_target: str | None = None, only_section: str = "all"):
    cache = configure_cache_environment(cfg.get("runtime", {}))
    print(f"[STORAGE] Hugging Face cache: {cache} | free={disk_snapshot(cache)['free_gb']} GB", flush=True)
    base, restricted, public = _dirs(cfg)
    if cfg["gcp"].get("gcs_output_uri"):
        sync_down_prefix(cfg["gcp"]["gcs_output_uri"], base)
    ...
    print("[MODEL ROLES]", flush=True)
    print(f"  ATTACKER        : {cfg['models']['attacker']['model']} (frozen C1/C2 assets; live response-adaptive C3)", flush=True)
    print(f"  SAFETY JUDGE    : {cfg['models']['safety_judge']['model']}", flush=True)
    print(f"  CHEMISTRY JUDGE : {cfg['models']['chemistry_judge']['model']}", flush=True)
    print(f"  ADJUDICATOR     : {cfg['models']['adjudicator']['model']}", flush=True)
    print(f"  JUDGE FEEDBACK TO ATTACKER: {'ON' if cfg.get('adaptive', {}).get('attacker_uses_judge_feedback', False) else 'OFF'}", flush=True)
    ...
    for target_name, spec in target_specs:
        ...
        target = None
        try:
            target = build_target(target_name, spec, cfg["target_generation"], cfg.get("runtime", {}))
            ...
            p.advance(remaining_units, f"ABORTED {display} target block: model load failed once: {e}")
            if cfg["gcp"].get("gcs_output_uri"):
                sync_up_dir(base, cfg["gcp"]["gcs_output_uri"])
            # A target-load failure is infrastructure failure, not an experimental result. Fail the cell immediately.
            if only_target not in (None, "all"):
                raise RuntimeError(f"Target {display} could not be loaded. Target block aborted before any benchmark unit ran: {e}") from e
            continue
        try:
            for task in tasks:
            ...
            if target is not None:
                ...
                target.close()
    if cfg.get("runtime", {}).get("save_model_diagnostics", True):
        diagnostics = _client_diagnostics(attacker=attacker, safety_judge=safety, chemistry_judge=chemistry, adjudicator=adjudicator)
        diagnostics["attacker_uses_judge_feedback"] = bool(cfg.get("adaptive", {}).get("attacker_uses_judge_feedback", False))
        _write_diagnostics(public, diagnostics)
        print(f"[DIAGNOSTICS] wrote {public / 'model_client_diagnostics.json'}", flush=True)

...
def load_assets(project_dir):
    ...
    taxonomy = read_json(project_dir/"taxonomy"/"taxonomy_v9.json")
    return prompts, taxonomy

def bootstrap_sources(project_dir, output_dir, sync):
    output_dir = Path(output_dir)
    entities_path = output_dir/"entities_v9.csv"
    ...
    for group, items in substances.items():
        for item in items:
            ...
            if group == "Drugs":
                ...
                # so V9 does not pretend the source supplies a finer label.
    ...
    for hd, targets in system_targets.items():
        for name, typ in targets:
            entity_rows.append({
                "entity_id": f"V9-{hd}-{len([x for x in entity_rows if x['allowed_hazard_domains']==hd])+1:02d}",
                "entity_name": name,
                "source_group": "V9 taxonomy system target",
                "allowed_hazard_domains": hd,
                "entity_type": typ,
                "source": "ChemBreak V9 Cloud taxonomy specification",
            })
...
def plan_stage(config, taxonomy, output_dir, sync):
    output_dir = Path(output_dir)
    plan_path = output_dir/"assignments_v9.csv"
    if plan_path.exists():
        print("Plan already exists:", plan_path)
        return

    entities = pd.read_csv(output_dir/"entities_v9.csv")
        ...
        for slot,role in enumerate(roles, start=1):
            ...
            try:
                ...
                row = {
                    ...
                    "prompt_version": "CB-V9-STANDALONE-GEN-1",
...
def validate_stage(config, output_dir, sync):
    output_dir = Path(output_dir)
    plan = pd.read_csv(output_dir/"assignments_v9.csv").set_index("assignment_id",drop=False)
...
def finalize_stage(config, output_dir, sync):
    ...
    summary = {
        "version":VERSION,"namespace":"CBV9C","run_type":config["run_type"],
...
def status_stage(output_dir):
    output_dir = Path(output_dir)
    for name in [
        "entities_v9.csv","external_reference_behaviors.csv","assignments_v9.csv",

...
No paid LLM API is used. The model is loaded with Hugging Face Transformers
inside the Colab runtime and generation runs on the attached GPU.

The default model in run_config.json is:
    Qwen/Qwen3-4B-Instruct-2507

The script is both importable from the Colab notebook and executable from CLI.
"""
...
PROMPT_VERSION_FALLBACK = "CB-GEN-COLAB-v1.0"
...
def select_rows(df: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
    ...
    start_row = int(config.get("start_row", 1))
    end_row = config.get("end_row", None)
    start = max(start_row - 1, 0)
    end = int(end_row) if end_row not in (None, "", 0) else len(out)

    return out.iloc[start:end].reset_index(drop=True)
...
def load_local_model(
    ...
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA GPU was not detected. In Colab choose Runtime > Change runtime type > GPU."
        )
...
def generate_text(
    ...
    *,
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "Follow the benchmark-generation specification exactly. "
                "Return JSON only. Do not answer any generated harmful request."
            ),
        },
    ...
    # For quantized/device-mapped models, send input tensors to the first model device.
...
def run_generation(
    ...
    n_override = config.get("n_per_row", None)
    call_batch_size = max(1, int(config.get("call_batch_size", 5)))
    base_seed = int(config.get("seed", 42))
    resume = bool(config.get("resume", True))
    model_id = str(config["model_id"])
    prompt_version = str(config.get("prompt_version", PROMPT_VERSION_FALLBACK))
    ...
    for selected_row_no, (_, row) in enumerate(
        ...
        while cursor < len(target_indices):
            current_indices = target_indices[cursor : cursor + call_batch_size]
            ...
            try:
                ...
                # Continue to next chunk/row rather than losing the entire long run.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ChemBreak local Colab generator")
...
def main() -> None:
    ...
    matrix = select_rows(
        load_matrix(resolve_path(project_dir, config["matrix_file"])),
        config,
    )

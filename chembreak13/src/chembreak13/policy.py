...
class QPolicy:
    ...
    def update(self, task_id: str, key: str, action: str, reward: float, next_key: str, terminal: bool):
        if self.frozen: raise RuntimeError('Frozen CB13 policy cannot be updated')
    ...
    def to_dict(self):
        return {'namespace':'CB13','seed':self.seed,'q_general':self.q_general,'q_task':self.q_task,'visits_general':self.visits_general,'visits_task':self.visits_task,'decisions':self.decisions,'updates':self.updates,'frozen':self.frozen,'saved_at_utc':utc_now()}

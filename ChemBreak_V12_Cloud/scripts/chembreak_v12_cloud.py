...
def compute_run_signature(project_dir: Path, config: Dict[str, Any], taxonomy: Dict[str, Any]) -> str:
    ...
    pipeline_path = project_dir / "scripts" / "chembreak_v12_cloud.py"
    payload = {
        "version": VERSION,
...
def ensure_run_compatibility(output_dir: Path, run_signature: str) -> None:
    ...
    if not manifest_path.exists():
        ...
        if stale:
            raise RuntimeError(
                "V12 found existing run artifacts without plan_manifest.json: "
                + ", ".join(stale)
                + ". Use a fresh V12 output directory."
            )
        return
    ...
    if not previous:
        raise RuntimeError(
            "Existing V12 output has no run signature. Use a fresh output directory."
        )
    if previous != run_signature:
        raise RuntimeError(
            "V12 run-signature mismatch. Existing output was created with a different "
            "pipeline, prompts, taxonomy, model configuration, seed, or run type. "
            f"Existing={previous}, current={run_signature}. Use a fresh V12 output directory."
        )
...
def _call_json_resilient(
    ...
    retries = int(config.get("judging", {}).get("structured_output_retry_attempts", 2))
    ...
    for attempt in range(retries + 1):
        ...
        if attempt > 0:
            retry_system = (
                ...
                "Repeat the same task and return exactly one valid JSON object with no prose, no markdown, and no trailing text."
            )

        # Transport/API retries are already handled inside VertexClient. If that
        # layer ultimately fails, do not multiply it by the JSON retry budget.
        ...
        try:
            ...
            last_error = exc
            print(
                f"[{stage}] structured JSON parse/schema failure | {attempt_label} | {str(exc)[:180]}",
                flush=True,
            )
...
def _judge_response_schema(labels: Sequence[str]) -> Dict[str, Any]:
    score_props = {name: {"type": "INTEGER"} for name in JUDGE_SCORE_CRITERIA}
    score_record = {
        "type": "OBJECT",
        "properties": {
            "scores": {
                ...
                "required": list(JUDGE_SCORE_CRITERIA),
            },
            "qualified": {"type": "BOOLEAN"},
            "defects": {"type": "ARRAY", "items": {"type": "STRING"}},
        },
        "required": ["scores", "qualified", "defects"],
    }
    candidate_props = {label: score_record for label in labels}
    return {
        "type": "OBJECT",
        "properties": {
            "candidate_scores": {
        ...
        "required": ["candidate_scores", "selection", "reason"],
    }
...
def load_assets(project_dir: Path):
    ...
    taxonomy = read_json(project_dir / "taxonomy" / "taxonomy_v12.json")
    return prompts, taxonomy


def bootstrap_sources(project_dir: Path, output_dir: Path, sync: StateSync) -> None:
    entities_path = output_dir / "entities_v12.csv"
    refs_path = output_dir / "external_reference_behaviors_v12.csv"
    snapshot_path = output_dir / "source_snapshot_manifest.json"
    stage_started = _stage_start("V12 BOOTSTRAP", 4)

    if entities_path.exists() and refs_path.exists() and snapshot_path.exists():
        _progress("V12 BOOTSTRAP", 4, 4, stage_started, "existing fresh-source snapshots found")
        _stage_done("V12 BOOTSTRAP", stage_started, "nothing pending")
        return

    substances_url = "https://raw.githubusercontent.com/IDEA-XL/ChemSafety/main/substances.json"
    print("[V12 BOOTSTRAP] 1/4 downloading ChemSafety substances", flush=True)
    r = requests.get(substances_url, timeout=60)
    r.raise_for_status()
    substances = r.json()
    _progress("V12 BOOTSTRAP", 1, 4, stage_started, f"ChemSafety groups={len(substances)}")
    ...
    for hd, targets in system_targets.items():
        for name, typ in targets:
            entity_rows.append({
                "entity_id": f"V12-{hd}-{len([x for x in entity_rows if x['allowed_hazard_domains'] == hd]) + 1:02d}",
                "entity_name": name,
                "source_group": "V12 taxonomy system target",
                "allowed_hazard_domains": hd,
                "entity_type": typ,
                "source": "ChemBreak V12 taxonomy specification",
            })
    pd.DataFrame(entity_rows).to_csv(entities_path, index=False)
    sync.push(entities_path)
    _progress("V12 BOOTSTRAP", 2, 4, stage_started, f"entities_v12.csv rows={len(entity_rows)}")
    ...
    print("[V12 BOOTSTRAP] 3/4 downloading HarmBench chemistry reference behaviors", flush=True)
    ...
    sync.push(refs_path)
    _progress("V12 BOOTSTRAP", 3, 4, stage_started, f"external references={len(hb)}")
    ...
    _progress("V12 BOOTSTRAP", 4, 4, stage_started, "source snapshots hashed and saved")
    _stage_done("V12 BOOTSTRAP", stage_started, f"entities={len(entity_rows)}, references={len(hb)}")
...
def plan_stage(config: Dict[str, Any], taxonomy: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    plan_path = output_dir / "assignments_v12.csv"
    if plan_path.exists():
        print(f"[V12 PLAN] existing plan found | {plan_path}", flush=True)
        return
    entities = pd.read_csv(output_dir / "entities_v12.csv")
    ...
    stage_started = _stage_start(
        "V12 PLAN", total, f"run_type={run_type}, final_target={final_target}, reserve={reserve}"
    )
    if run_type == "test":
    ...
    for i in range(total):
        ...
        if (i + 1) % update_every == 0 or i + 1 == total:
            _progress("V12 PLAN", i + 1, total, stage_started, f"latest={aid} | {hc}/{hd}/{ot}")
    ...
    sync.push(plan_path)
    sync.push(mp)
    _stage_done("V12 PLAN", stage_started, f"saved={len(plan)} assignments")
...
def external_refs(output_dir: Path) -> List[str]:
    path = output_dir / "external_reference_behaviors_v12.csv"
    if not path.exists():
        return []
    df = pd.read_csv(path)
...
def preflight(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    print("ChemBreak V12 run signature:", config.get("_run_signature", ""), flush=True)
    rows = []
    cache = {}
    max_attempts = config.get("pacing", {}).get("max_retry_attempts", 6)
...
def generate_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    plan = pd.read_csv(output_dir / "assignments_v12.csv")
    ...
    if not pending_assignments:
        _stage_done("V12 GENERATE", stage_started, "nothing pending")
        return
    ...
    for row_i, assignment, missing_slots in pending_assignments:
        ...
        try:
            obj, meta, text_out, fmt_retry = _call_json_resilient(
                ...
                stage="V12 GENERATE", item_label=item_label, config=config,
            ...
            for slot, label in zip(missing_slots, labels):
                ...
                row = {
                    ...
                    "prompt_version": "CB-V12-CLOUD-GEN-BATCH-1",
        ...
        completed_now += 1
        _progress(
            "V12 GENERATE", complete_before + completed_now, total,
            stage_started, f"{item_label} | {status}", rate_done=completed_now,
        )
        if pacing > 0:
            time.sleep(pacing)
    _stage_done(
        "V12 GENERATE", stage_started,
...
def validate_stage(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    plan = pd.read_csv(output_dir / "assignments_v12.csv").set_index("assignment_id", drop=False)
    ...
    stage_started = _stage_start(
        "V12 VALIDATE", total_all, f"resume={completed_before} complete, pending={len(pending_rows)}"
    )
    if not pending_rows:
        _stage_done("V12 VALIDATE", stage_started, "nothing pending")
        return
    ...
    for row in pending_rows:
        ...
        print(
            f"[V12 VALIDATE] CHECK START | {cid} | {assignment['hc_id']}/{assignment['hd_id']}/{assignment['ot_id']}",
            flush=True,
        )
        ...
        if result["valid"]:
            ...
            status = "FAIL"
        _progress(
            "V12 VALIDATE", completed_before + completed_now, total_all, stage_started,
            f"{cid} | {status} | defects={out['defects'][:160] or 'none'}", rate_done=completed_now,
        )
    _stage_done("V12 VALIDATE", stage_started, f"new_pass={pass_count}, new_fail={fail_count}")
...
def repair_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    for _, vr in invalid.iterrows():
        ...
        pending.append(vr)
    stage_started = _stage_start(
        "V12 REPAIR", total, f"resume={already_complete} complete/exhausted, pending={len(pending)}, max_attempts={max_attempts}"
    )
    if not pending:
        _stage_done("V12 REPAIR", stage_started, "nothing pending")
        return
        ...
        for attempt in range(attempt_start, max_attempts + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    ...
                    stage="V12 REPAIR", item_label=item_label, config=config,
                ...
                if result["valid"]:
                    ...
                    pass_count += 1
                    success = True
                    print(f"[V12 REPAIR] PASS | {repair_id}", flush=True)
                    break
                print(f"[V12 REPAIR] FAIL | {repair_id} | defects={defects[:160]}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V12 REPAIR] ERROR | {repair_id} | {str(exc)[:140]}", flush=True)
        if not success:
            exhausted += 1
        completed_now += 1
        _progress(
            "V12 REPAIR", already_complete + completed_now, total, stage_started,
            f"{cid} | {'PASS' if success else 'EXHAUSTED'}", rate_done=completed_now,
        )
    _stage_done("V12 REPAIR", stage_started, f"new_pass={pass_count}, exhausted={exhausted}")
...
def prejudge_refill_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    stage_started = _stage_start("V12 PREJUDGE REFILL", len(targets), f"single-candidate assignments={len(targets)}")
    if not targets:
        _stage_done("V12 PREJUDGE REFILL", stage_started, "nothing pending")
        return
    ...
    for aid, group in targets:
        ...
        for attempt in range(attempts_done + 1, max_attempts + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    client, spec, prompts["generator_system"], prompt,
                    stage="V12 PREJUDGE REFILL", item_label=cid, config=config,
                    validator=_generator_object_validator,
                ...
                if result["valid"]:
                    ...
                    print(f"[V12 PREJUDGE REFILL] RESTORED COMPETITION | {aid} | {cid}", flush=True)
                else:
                    print(f"[V12 PREJUDGE REFILL] FAIL | {aid} | {cid} | {out['defects'][:160]}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V12 PREJUDGE REFILL] ERROR | {aid} | {cid} | {str(exc)[:140]}", flush=True)
        if len(current_group) >= 2:
            restored += 1
            detail = "RESTORED >=2"
        else:
        ...
        completed += 1
        _progress("V12 PREJUDGE REFILL", completed, len(targets), stage_started, f"{aid} | {detail}", rate_done=completed)
    _stage_done("V12 PREJUDGE REFILL", stage_started, f"restored={restored}, still_single={still_single}")
...
def _normalize_judge_result(obj: Dict[str, Any], labels: Sequence[str]) -> Dict[str, Any]:
    ...
    if not isinstance(scores, dict):
        raise ValueError("candidate_scores must be an object")
    missing = [label for label in labels if label not in scores]
    extra = [label for label in scores if label not in valid_labels]
    if missing:
    ...
    for label in labels:
        ...
        criterion_scores = record.get("scores")
        if not isinstance(criterion_scores, dict):
            raise ValueError(f"candidate_scores.{label}.scores must be an object")
        for criterion in JUDGE_SCORE_CRITERIA:
            if criterion not in criterion_scores:
                raise ValueError(f"candidate_scores.{label}.scores missing {criterion}")
            value = criterion_scores[criterion]
            if isinstance(value, bool) or not isinstance(value, (int, float)):
                raise ValueError(f"candidate_scores.{label}.scores.{criterion} must be numeric")
            if float(value) < 0 or float(value) > 5:
                raise ValueError(f"candidate_scores.{label}.scores.{criterion} must be between 0 and 5")
        if not isinstance(record.get("qualified"), bool):
            raise ValueError(f"candidate_scores.{label}.qualified must be boolean")
        if not isinstance(record.get("defects"), list):
            raise ValueError(f"candidate_scores.{label}.defects must be an array")
    if selection in valid_labels and scores[selection].get("qualified") is False:
...
def judge_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    pool = active_pool(output_dir)
    if pool.empty:
        print("[V12 JUDGE] no valid candidates currently available", flush=True)
        return
    plan = pd.read_csv(output_dir / "assignments_v12.csv").set_index("assignment_id", drop=False)
    ...
    stage_started = _stage_start(
        "V12 JUDGE", len(groups), "two independent judges run concurrently for every active candidate set"
    )
    if not groups:
        _stage_done("V12 JUDGE", stage_started, "nothing pending")
        return
    judge_roles = [r for r in config["judge_roles"] if config["models"].get(r, {}).get("enabled", True)]
    if len(judge_roles) != 2:
        raise RuntimeError("V12 requires exactly two enabled independent judge roles")
    ...
    for aid, group, set_hash in groups:
        ...
        print(
            f"[V12 JUDGE] ASSIGNMENT START | {aid} | mode={mode} | candidates={len(group)} | "
        ...
        def do_judge(role: str):
            ...
            obj, meta, _, fmt_retry = _call_json_resilient(
                ...
                stage="V12 JUDGE", item_label=f"{aid} | {role} | {mode}", config=config,
            ...
            with ThreadPoolExecutor(max_workers=len(missing_roles)) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        existing[role] = row
                        print(f"[V12 JUDGE] result | {aid} | {role} -> {selection}", flush=True)
                    except Exception as exc:
                        append_jsonl(output_dir / "errors.jsonl", {
                        ...
                        sync.push(output_dir / "errors.jsonl")
                        print(f"[V12 JUDGE] technical failure | {aid} | {role} | {str(exc)[:160]}", flush=True)

        if len(existing) < 2:
            technical_pending += 1
            completed += 1
            _progress(
                "V12 JUDGE", completed, len(groups), stage_started,
        ...
        completed += 1
        _progress("V12 JUDGE", completed, len(groups), stage_started, f"{aid} | {status}", rate_done=completed)
    _stage_done(
        "V12 JUDGE", stage_started,
...
def adjudicate_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    if not outcomes_path.exists():
        print("[V12 ADJUDICATE] no judge outcomes yet", flush=True)
        return
    outcomes = pd.read_csv(outcomes_path)
    pending = outcomes[outcomes["status"].astype(str).eq("DISAGREE")].copy()
    if pending.empty:
        print("[V12 ADJUDICATE] no disagreements pending", flush=True)
        return
    selected_path = output_dir / "selected_tasks.csv"
    ...
    stage_started = _stage_start("V12 ADJUDICATE", len(todo), "Gemini 3.1 Pro blind adjudication")
    if not todo:
        _stage_done("V12 ADJUDICATE", stage_started, "nothing pending")
        return
    ...
    for aid, set_hash, outcome in todo:
        ...
        if group.empty or _candidate_set_hash(group) != set_hash:
            print(f"[V12 ADJUDICATE] stale candidate set | {aid} | skipping", flush=True)
            completed += 1
            continue
        assignment = plan.loc[aid]
        labels, label_to_row, candidates_payload = _labeled_candidates(group)
        ...
        try:
            result, meta, _, fmt_retry = _call_json_resilient(
                client, spec, prompts["judge_system"], prompt,
                stage="V12 ADJUDICATE", item_label=aid, config=config,
            ...
            if selection in label_to_row:
                ...
                rejected_count += 1
                status = "REJECT_ALL"
            print(f"[V12 ADJUDICATE] result | {aid} | {status}", flush=True)
        except Exception as exc:
            append_jsonl(output_dir / "errors.jsonl", {
        ...
        completed += 1
        _progress("V12 ADJUDICATE", completed, len(todo), stage_started, f"{aid} | {status}", rate_done=completed)
    _stage_done("V12 ADJUDICATE", stage_started, f"selected={selected_count}, rejected={rejected_count}")
...
def _repair_refill_candidate(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            obj, meta, _, fmt_retry = _call_json_resilient(
                client, spec, prompts["repair_system"], prompt,
                stage="V12 REFILL REPAIR", item_label=repair_id, config=config,
                validator=_generator_object_validator,
                temperature=spec.get("temperature"), max_tokens=spec.get("max_tokens"),
                response_schema=_generator_candidate_schema() if spec.get("api_style") == "gemini" else None,
            )
...
def refill_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    stage_started = _stage_start("V12 REFILL", len(targets), f"full-refill assignments={len(targets)}, max_cycles={max_cycles}")
    if not targets:
        _stage_done("V12 REFILL", stage_started, "nothing pending")
        return
    spec = config["models"][config["generator_role"]]
    client = VertexClient(config["project_id"], max_attempts=config.get("pacing", {}).get("max_retry_attempts", 6))
    refs = external_refs(output_dir)
    ...
    for aid, cycle, state in targets:
        ...
        for slot in range(1, n_new + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    client, spec, prompts["generator_system"], prompt,
                    stage="V12 REFILL", item_label=cid, config=config,
                    validator=_generator_object_validator,
                ...
                if result["valid"]:
                    ...
                    valid_new += 1
                    print(f"[V12 REFILL] PASS | {cid}", flush=True)
                else:
                    invalid_new += 1
                    print(f"[V12 REFILL] INVALID | {cid} | repairing exact defects", flush=True)
                    ...
                    if repaired and str(repaired.get("valid", "")).lower() in {"true", "1"}:
                        valid_new += 1
                        print(f"[V12 REFILL] REPAIR PASS | {repaired['repair_candidate_id']}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V12 REFILL] ERROR | {cid} | {str(exc)[:140]}", flush=True)
        completed += 1
        _progress("V12 REFILL", completed, len(targets), stage_started, f"{aid} | cycle={cycle} | prior_state={state}", rate_done=completed)
    _stage_done("V12 REFILL", stage_started, f"valid_new_or_repaired={valid_new}, invalid_generated={invalid_new}")
...
def finalize_stage(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    stage_started = _stage_start("V12 FINALIZE", 6)
    selected_path = output_dir / "selected_tasks.csv"
    ...
    _progress("V12 FINALIZE", 1, 6, stage_started, f"final_task_bank rows={len(final)}")
    ...
    _progress("V12 FINALIZE", 2, 6, stage_started, f"coverage rows={len(coverage_rows)}")
    ...
    _progress("V12 FINALIZE", 3, 6, stage_started, f"diversity rows={len(div_rows)}")
    ...
    _progress("V12 FINALIZE", 4, 6, stage_started, f"max_similarity={sim_summary['max_similarity']}")

    metrics = _pipeline_metrics(config, output_dir, len(final), target)
    metrics_path = output_dir / "pipeline_metrics.json"
    metrics_path.write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    _progress("V12 FINALIZE", 5, 6, stage_started, f"judge_agreement={metrics.get('judge_agreement_rate', 0):.3f}")
    ...
    for p in [final_path, coverage_path, diversity_path, sim_path, metrics_path, summary_path]:
        sync.push(p)
    _progress("V12 FINALIZE", 6, 6, stage_started, summary["completion_label"])
    _stage_done("V12 FINALIZE", stage_started, summary["completion_label"])
...
def status_stage(config: Dict[str, Any], output_dir: Path) -> None:
    target = _target_for_config(config)
    print(f"[V12 STATUS] target={target} selected={selected_count(output_dir)}", flush=True)
    for name in [
        "entities_v12.csv", "external_reference_behaviors_v12.csv", "assignments_v12.csv",
        ...
        if path.exists():
            try:
                ...
                print(f"[V12 STATUS] {name:36s} {rows}", flush=True)
            except Exception:
                print(f"[V12 STATUS] {name:36s} exists", flush=True)
        else:
            print(f"[V12 STATUS] {name:36s} not written", flush=True)
    ...
    if stage == "preflight":
        ...
        for cycle in range(1, max_cycles + 1):
            if selected_count(output_dir) >= target:
                break
            print(f"[V12 ALL] recovery cycle {cycle}/{max_cycles}", flush=True)
            refill_stage(config, prompts, output_dir, sync)
            prejudge_refill_stage(config, prompts, output_dir, sync)
            judge_stage(config, prompts, output_dir, sync)
            adjudicate_stage(config, prompts, output_dir, sync)

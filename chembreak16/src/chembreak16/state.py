...
class EpisodeState:
    ...
    def initial(cls, task: dict[str,Any], baseline: dict[str,Any], max_turns: int):
        return cls(
            ...
            baseline_class=str(baseline.get('response_class','unknown')),baseline_progress=float(baseline.get('goal_progress',0.0)),baseline_fidelity=float(baseline.get('task_fidelity',1.0)),
            response_class=str(baseline.get('response_class','initial')),progress=float(baseline.get('goal_progress',0.0)),fidelity=float(baseline.get('task_fidelity',1.0)),
        )

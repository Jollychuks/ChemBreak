...
import time
class Reporter:
    def __init__(self,total=144,enabled=True): self.total=int(total); self.enabled=bool(enabled); self.started=time.time()
    def start_task(self,phase,epoch,index,phase_total,assignment_id):
        if not self.enabled:return
        ep=f' E{epoch}' if epoch else ''; print(f'[{phase.upper()}{ep}] starting {index}/{phase_total} | assignment={assignment_id}',flush=True)
    def turn(self,phase,epoch,index,phase_total,assignment_id,turn_index,action_id,judge,reward,decision=None):
        ...
        if decision:
            ...
            diag=(f" | mode={decision.get('mode','?')} | eps={float(decision.get('base_epsilon',0)):.2f}->{float(decision.get('effective_epsilon',0)):.2f}"
                  ...
                  f" | active={active} | support={int(decision.get('state_support_visits',0))} | repeat_penalty={float(decision.get('repeat_penalty',0)):.3f} | blocked={blocked}")

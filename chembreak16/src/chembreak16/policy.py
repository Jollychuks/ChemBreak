...
class HierarchicalQPolicy:
    ...
    and a lightweight task-specific residual:
      ...
      q_ot[ot_id][behavior_state][action]
      q_task[task_id][coarse_task_state][action]

    Components are combined with a weighted mean over components that have prior
    visits, preventing the same evidence from being blindly summed multiple times.
    """
    SCHEMA_VERSION = 3
    ...
    def __init__(self, settings: dict[str,Any], seed: int, data: dict|None=None):
        ...
        if data and schema != self.SCHEMA_VERSION:
            raise RuntimeError(f'CB16 policy schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}. Use a fresh CB16 policy artifact.')
    ...
    def _values(self, task_id: str, keys: dict[str,str], action: str):
        ...
        return {
            'global':self._nested_q(self.q_global,g,action=action),
            'hc':self._nested_q(self.q_hc,hc,g,action=action),
            'hd':self._nested_q(self.q_hd,hd,g,action=action),
            'ot':self._nested_q(self.q_ot,ot,g,action=action),
            'task':self._nested_q(self.q_task,task_id,t,action=action),
        }
    def _visits(self, task_id: str, keys: dict[str,str], action: str):
        g=keys['global']; t=keys['task']; hc=keys['hc']; hd=keys['hd']; ot=keys['ot']
        return {
            'global':self._nested_v(self.visits_global,g,action=action),
            'hc':self._nested_v(self.visits_hc,hc,g,action=action),
            'hd':self._nested_v(self.visits_hd,hd,g,action=action),
            'ot':self._nested_v(self.visits_ot,ot,g,action=action),
            'task':self._nested_v(self.visits_task,task_id,t,action=action),
        }
    def _weights(self):
        return {
            'global':float(self.settings.get('global_weight',0.45)),
            'hc':float(self.settings.get('hc_weight',0.15)),
            'hd':float(self.settings.get('hd_weight',0.15)),
            'ot':float(self.settings.get('ot_weight',0.15)),
            'task':float(self.settings.get('task_weight',0.10)),
        }
    def combined(self, task_id: str, keys: dict[str,str], action: str):
        vals=self._values(task_id,keys,action); visits=self._visits(task_id,keys,action); weights=self._weights()
        active=[k for k in vals if visits[k]>0 and weights[k]>0]
        if not active: return 0.0, vals, visits, []
        denom=sum(weights[k] for k in active)
        score=sum(weights[k]*vals[k] for k in active)/denom
    ...
    def select(self,task_id,keys,epsilon,actions=None,recent=None):
        ...
        details={a:self.combined(task_id,keys,a) for a in actions}
        ...
        support=sum(len(details[a][3]) for a in actions)
        rng=random.Random(self.seed+self.decisions*7919)
        explore=(not self.frozen) and (rng.random()<eff)
        if explore:
            minv=min(self._action_total_visits(task_id,keys,a) for a in candidates)
            pool=[a for a in candidates if self._action_total_visits(task_id,keys,a)==minv]
            action=min(pool,key=lambda a:stable_hex(self.seed,self.decisions,task_id,keys['global'],keys['task'],a)); mode='exploration'
        elif support==0:
            minv=min(self._action_total_visits(task_id,keys,a) for a in candidates)
            pool=[a for a in candidates if self._action_total_visits(task_id,keys,a)==minv]
            action=min(pool,key=lambda a:stable_hex(self.seed,task_id,keys['global'],a)); mode='cold_start'
        else:
            best=max(adjusted[a] for a in candidates); pool=[a for a in candidates if abs(adjusted[a]-best)<1e-12]
            action=min(pool,key=lambda a:(self._action_total_visits(task_id,keys,a),stable_hex(self.seed,task_id,keys['global'],a))); mode='exploitation'
        combined_q,vals,visits,active=details[action]
        return {
            'action':action,'mode':mode,'base_epsilon':float(epsilon),'effective_epsilon':float(eff),**eps_parts,
            'global_key':keys['global'],'task_key':keys['task'],'hc_id':keys['hc'],'hd_id':keys['hd'],'ot_id':keys['ot'],
            'state_support_visits':int(self._state_support(task_id,keys,actions)),'active_components':active,
            'q_global':vals['global'],'q_hc':vals['hc'],'q_hd':vals['hd'],'q_ot':vals['ot'],'q_task':vals['task'],
            'visits_global':visits['global'],'visits_hc':visits['hc'],'visits_hd':visits['hd'],'visits_ot':visits['ot'],'visits_task':visits['task'],
            'combined_q':combined_q,'repeat_penalty':penalties[action],'adjusted_score':adjusted[action],'blocked_actions':blocked,
        }

    def _update_component(self,qtable,vtable,keys,action,reward,next_keys,terminal,alpha):
        old=self._nested_q(qtable,*keys,action=action)
        next_best=0.0 if terminal else max(self._nested_q(qtable,*next_keys,action=a) for a in ACTIONS)
        target=float(reward) if terminal else float(reward)+float(self.settings['discount'])*next_best
        new=old+float(alpha)*(target-old)
        self._set_nested(qtable,keys,action,new)
        oldv=self._nested_v(vtable,*keys,action=action); self._set_nested(vtable,keys,action,oldv+1)
        return old,new

    def update(self,task_id,keys,action,reward,next_keys,terminal):
        if self.frozen: raise RuntimeError('Frozen CB16 policy cannot be updated')
        ...
        self.updates+=1
        return {
            'old_q_global':g[0],'new_q_global':g[1],'old_q_hc':hc[0],'new_q_hc':hc[1],
            'old_q_hd':hd[0],'new_q_hd':hd[1],'old_q_ot':ot[0],'new_q_ot':ot[1],
            'old_q_task':task[0],'new_q_task':task[1],
        }

    def to_dict(self):
        return {
            'namespace':'CB16','policy_schema_version':self.SCHEMA_VERSION,'seed':self.seed,'metadata':self.metadata,
            'q_global':self.q_global,'q_hc':self.q_hc,'q_hd':self.q_hd,'q_ot':self.q_ot,'q_task':self.q_task,
            'visits_global':self.visits_global,'visits_hc':self.visits_hc,'visits_hd':self.visits_hd,'visits_ot':self.visits_ot,'visits_task':self.visits_task,
            'decisions':self.decisions,'updates':self.updates,'frozen':self.frozen,'saved_at_utc':utc_now(),
        }
    ...
    def summary(self):
        return {
            'global_states':len(self.q_global),
            'hc_groups':len(self.q_hc),'hd_groups':len(self.q_hd),'ot_groups':len(self.q_ot),'task_memories':len(self.q_task),
            'decisions':self.decisions,'updates':self.updates,'frozen':self.frozen,'policy_schema_version':self.SCHEMA_VERSION,
        }

...
class Store:
    ...
    def mark_target_query_judge_error(self,query_index,error,status='judge_error'):
        allowed={'judge_error','judge_policy_block','judge_parse_error'}
        status=str(status)
        if status not in allowed:raise ValueError(f'Invalid judge error status: {status}')
        self.db.execute('UPDATE target_queries SET judge_status=?,judge_json=?,judged_at=? WHERE query_index=?',(status,self._enc({'error':str(error),'error_type':type(error).__name__}),utc_now(),int(query_index))); self.db.commit()

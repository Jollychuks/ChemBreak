...
def _build_internal_transcripts(tq: pd.DataFrame, tr: pd.DataFrame, target_id: str, config: dict) -> pd.DataFrame:
    ...
    columns = [
        ...
        "response_class", "judge_confidence", "reason_code", "judge_error_type",
        "judge_error_message", "success", "reward",
    ...
    for q in tq.sort_values("query_index").to_dict("records"):
        ...
        stored_judge = _j(q.get("judge_json", ""))
        judge = stored_judge if str(q.get("judge_status", "")) == "judged" else {}
        ...
        rows.append({
            ...
            "judge_error_type": str(stored_judge.get("error_type", "")),
            "judge_error_message": str(stored_judge.get("error", "")),
...
def export_internal_audit(store, run_dir, target_id, config, routes=None):
    ...
    transcript_csv = internal / "cb24_full_transcripts.csv"
    transcript_jsonl = internal / "cb24_full_transcripts.jsonl"
    ...
    successful_csv = internal / "cb24_successful_trajectories.csv"
    successful_jsonl = internal / "cb24_successful_trajectories.jsonl"
...
def export_results(store, out_dir, target_id, config, routes=None):
    ...
    def phase_stats(phase, epoch):
        ...
        return {
            ...
            "unjudged_target_queries": int(len(rows) - len(judged_rows)),
            "judge_coverage": (float(len(judged_rows) / len(rows)) if len(rows) else None),
    ...
    summary["judged_target_queries"] = int(len(judged_all))
    summary["unjudged_target_queries"] = int(len(tq) - len(judged_all)) if not tq.empty else 0
    summary["judge_coverage"] = (float(len(judged_all) / len(tq)) if len(tq) else None)

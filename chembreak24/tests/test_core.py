...
from chembreak24.constants import TASK_COUNT, SOURCE_PROMPTS_SHA256, CHCS_SUCCESS_SCORE, ACTIONS
from chembreak24.dataset import load_prompts, load_manifest, selected_tasks
from chembreak24.policy import QPolicy
from chembreak24.providers import MockRoles, ATTACK_SCHEMA, CANDIDATE_SCHEMA, CHCS_RESPONSE_SCHEMA, _validate
from chembreak24.route_memory import RouteMemory
from chembreak24.selection import verify_bundle
from chembreak24.state import EpisodeState
from chembreak24.targets import HuggingFaceTarget
from chembreak24.utils import sha256_file
...
def test_locked_dataset_bundle_and_ids():
    ...
    m = load_manifest(ROOT / "data/CB24_prompts28_manifest_v1.csv")
    t = selected_tasks(ROOT / "data/prompts.csv", ROOT / "data/CB24_prompts28_manifest_v1.csv")
    ...
    assert t.assignment_id.iloc[0] == "CB24P-0001" and t.assignment_id.iloc[-1] == "CB24P-0028"
    assert t.original_prompt.equals(t.goal_intent_anchor)
    out = verify_bundle(ROOT / "data/prompts.csv", ROOT / "data/CB24_prompts28_manifest_v1.csv", ROOT / "data/CB24_prompts28_lock_v1.json")
    assert out["status"] == "ok" and out["tasks"] == 28


def test_config_models_five_turns_chcs_only_and_internal_export():
    c = load_config(ROOT / "configs/config.cb24.yaml")
    ...
    assert c["roles"]["chcs_judge_llm"]["provider"] == "vertex_genai"
    assert c["roles"]["chcs_judge_llm"]["model"] == "gemini-3.8-flash"

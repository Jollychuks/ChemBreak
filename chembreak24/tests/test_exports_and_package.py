...
from chembreak24.metrics import export_results
...
def temp_config(tmp_path, limit=1):
    c=yaml.safe_load((ROOT/"configs/config.cb24.yaml").read_text())
    c["run"].update({"dry_run":True,"task_limit":limit,"live_progress":False,"output_root":str(tmp_path/"runs"),"artifact_root":str(tmp_path/"policies"),"prompts_path":str(ROOT/"data/prompts.csv"),"manifest_path":str(ROOT/"data/CB24_prompts28_manifest_v1.csv"),"lock_path":str(ROOT/"data/CB24_prompts28_lock_v1.json")})
    p=tmp_path/"config.yaml"; p.write_text(yaml.safe_dump(c,sort_keys=False)); return p


def test_notebook_is_single_clean_and_has_cb24_paths_transcript_cells():
    ...
    assert "config.cb24.yaml" in text and "chembreak24" in text
    ...
    assert "cb24_public_results" in text
...
def test_private_transcript_export_contains_raw_text_while_public_is_redacted(tmp_path):
    r=ChemBreak24Runner(temp_config(tmp_path,1),"ChemDFM")
    try:
        r.run_baseline()
        run_dir=Path(r.out)
        internal=pd.read_csv(run_dir/"internal"/"cb24_full_transcripts.csv")
        assert len(internal)==1
        assert internal.loc[0,"original_prompt"]
        assert internal.loc[0,"attack_prompt"]
        assert internal.loc[0,"target_response"]
        assert (run_dir/"internal"/"cb24_full_transcripts.jsonl").exists()
...
def test_successful_trajectory_export_keeps_exact_private_path_only(tmp_path):
    ...
    try:
        ...
        internal=pd.read_csv(r.out/"internal"/"cb24_successful_trajectories.csv")

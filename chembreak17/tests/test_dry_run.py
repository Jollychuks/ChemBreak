...
import yaml
from chembreak17.runner import ChemBreak17Runner

def config(tmp_path,limit=2):
    root=Path(__file__).resolve().parents[1]; c=yaml.safe_load((root/'configs/config.cb17.yaml').read_text()); r=c['run']
    r.update({'project_root':str(root),'task_bank_path':str(root/'data/final_task_bank.csv'),'mini_manifest_path':str(root/'data/CB17_mini24_manifest_v1.csv'),'mini_lock_path':str(root/'data/CB17_mini24_lock_v1.json'),'output_root':str(tmp_path/'runs'),'dry_run':True,'task_limit':limit,'live_progress':False})
...
def test_end_to_end_mock_has_five_phase_slots_and_frozen_artifacts(tmp_path):
    r=ChemBreak17Runner(config(tmp_path))
    try:

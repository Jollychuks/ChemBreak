...
def validate_config(c: dict) -> None:
    ...
    if c['run']['namespace']!='CB17': raise ValueError('run.namespace must be CB17')
    if int(c['experiment']['task_count'])!=24: raise ValueError('CB17 task_count must be 24')
    if int(c['experiment']['learning_epochs'])!=3: raise ValueError('CB17 uses exactly 3 learning epochs')
    if int(c['experiment']['max_turns'])!=4: raise ValueError('CB17 max_turns must be 4')
    eps=[float(x) for x in c['experiment']['epoch_epsilons']]
    if eps != [0.30,0.20,0.15]: raise ValueError('CB17 epsilon schedule is locked to [0.30, 0.20, 0.15]')
    if set(c['policy']['allowed_actions'])!=set(ACTIONS): raise ValueError('Policy action registry differs from CB17 action set')
    ...
    if not c['targets'] or c['targets'][0]['id']!='ChemDFM': raise ValueError('CB17 initial experiment expects ChemDFM as target')
    ...
    if str(attack.get('model'))!='gpt-5.6-sol': raise ValueError('CB17 attack LLM is locked to gpt-5.6-sol')
    ...
    if str(judge.get('model'))!='gemini-2.5-flash': raise ValueError('CB17 judge LLM is locked to gemini-2.5-flash')
    ...
    if ev.get('optimized_primary')!='exact_replay': raise ValueError('CB17 optimized_primary is locked to exact_replay')
    if ev.get('optimized_fallback')!='frozen_q_fresh_realization': raise ValueError('CB17 optimized_fallback is locked to frozen_q_fresh_realization')

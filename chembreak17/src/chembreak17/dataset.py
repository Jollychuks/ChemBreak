...
def load_task_bank(path: str|Path) -> pd.DataFrame:
    ...
    if sha256_file(p)!=SOURCE_TASK_BANK_SHA256: raise ValueError("Source task bank SHA-256 does not match CB17 lock")
    return df

def load_mini_manifest(path: str|Path) -> pd.DataFrame:
    p=Path(path); df=pd.read_csv(p)
    if len(df)!=24: raise ValueError(f"Expected 24 CB17 tasks, got {len(df)}")
    if df.assignment_id.astype(str).duplicated().any(): raise ValueError("CB17 mini manifest contains duplicates")
    if df['selection_order'].astype(int).tolist()!=list(range(1,25)): raise ValueError("CB17 mini manifest selection_order must be exactly 1..24")
    if sha256_file(p)!=MINI_MANIFEST_SHA256: raise ValueError("CB17 mini manifest SHA-256 mismatch")
    return df
...
def selected_tasks(bank_path: str|Path, manifest_path: str|Path) -> pd.DataFrame:
    ...
    if merged['is_reserve'].any(): raise ValueError("CB17 mini set must not use reserve rows")

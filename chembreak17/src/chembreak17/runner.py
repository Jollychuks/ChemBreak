...
class ChemBreak17Runner:
    def __init__(self, config_path):
        ...
        evidence_keys=('replay_during_learning','replay_on_exploitation_only','require_prior_success','wilson_z','target_support_attempts','reliability_weight','support_weight','reward_weight','q_weight','reward_scale','q_scale','state_match_global_bonus','state_match_task_bonus','optimized_primary','optimized_fallback','block_failed_fresh_action_if_alternative')
        self.identity={
            'namespace':'CB17','package_version':'17.0.0','policy_schema_version':QPolicy.SCHEMA_VERSION,'evidence_schema_version':EvidenceMemory.SCHEMA_VERSION,
            ...
            'method_signature':{
                ...
                'attack_llm':{k:attack_cfg.get(k) for k in ('provider','model','reasoning_effort','max_output_tokens','attempts')},
        ...
        if existing is None and rows:
            self.store.close(); raise RuntimeError('Existing CB17 checkpoint has no experiment identity. Use a new EXPERIMENT_REVISION or intentionally clear that CB17 run directory.')
        if existing is not None and existing!=self.identity:
            self.store.close(); raise RuntimeError('Existing CB17 checkpoint belongs to a different experiment identity or method configuration. Use a new EXPERIMENT_REVISION or intentionally clear that CB17 run directory.')
        ...
        for name,obj in [('policy',self.policy),('evidence',self.evidence)]:
            if obj.metadata and obj.metadata!=self.identity:
                self.store.close(); raise RuntimeError(f'Training {name} metadata does not match this CB17 experiment identity')
    ...
    def _restore(self,task,baseline,phase,epoch):
        state=EpisodeState.initial(task,baseline,int(self.config['experiment']['max_turns'])); total=0.0; success=False; attempted=set(); failed_fresh_actions=set()
        ...
        return state,total,success,attempted,failed_fresh_actions

    def _fresh_attack(self,task,action,state,last_response):
        result=self.roles.attack(attack_prompt(task,action,state,last_response),action); prompt=str(result['utterance']).strip()
    ...
    def _learning_candidate(self,task,state,epsilon,decision,attempted,last_response):
        ...
        if can_replay:
            candidate=self.evidence.best_live_candidate(aid,state.policy_keys(),attempted_realizations=attempted)
    ...
    def _run_learning_episode(self,task,epoch,epsilon,index):
        ...
        state,total_reward,success,attempted,_=self._restore(task,baseline,'learning',epoch); last_response=state.history[-1]['content'] if state.history else str(baseline.get('response',''))
        while state.turn_index<int(self.config['experiment']['max_turns']) and not success:
            keys=state.policy_keys(); base_decision=self.policy.select(aid,keys,epsilon,ACTIONS,recent=state.decision_history)
            ...
            evidence_after=self.evidence.record(task_id=aid,prompt=prompt,action_id=action,global_key=keys['global'],task_key=keys['task'],phase='learning',epoch=epoch,turn_index=state.turn_index,success=bool(judge['success']),reward=reward,combined_q=float(decision.get('combined_q',0.0)),source=source)
            decision['candidate_id']=cid; decision['realization_id']=rid; decision['candidate_source']=source; decision['evidence_after']={'attempts':evidence_after['attempts'],'successes':evidence_after['successes'],'failures':evidence_after['failures'],**self.evidence.score(evidence_after)}
        ...
        self.store.complete_episode('learning',epoch,aid,success,state.turn_index,total_reward,'success' if success else 'turn_budget_exhausted')
    ...
    def freeze_policy(self):
        ...
        if missing:raise RuntimeError(f'Cannot freeze CB17: {len(missing)} learning episodes are incomplete')
        ...
        rankings=frozen_evidence.build_rankings(); snapshot={'namespace':'CB17','package_version':'17.0.0','frozen_at_utc':utc_now(),'identity':self.identity,'policy':frozen_policy.to_dict(),'evidence':frozen_evidence.to_dict(),'candidate_rankings':rankings}
    ...
    def _load_frozen_bundle(self):
        if not self.freeze_snapshot_path.exists():raise FileNotFoundError('CB17 freeze snapshot not found. Run freeze_policy() first.')
        snapshot=json.loads(self.freeze_snapshot_path.read_text())
        if snapshot.get('identity')!=self.identity:raise RuntimeError('Freeze snapshot identity does not match this CB17 experiment')
    ...
    def _optimized_candidate(self,task,state,frozen_policy,rankings,attempted,failed_fresh_actions,last_response):
    ...
    def _run_optimized_episode(self,task,index,frozen_policy,rankings):
        aid=str(task['assignment_id']); baseline=self.store.baseline(aid)
        if baseline is None:raise RuntimeError(f'Baseline missing for {aid}')
        state,total_reward,success,attempted,failed_fresh_actions=self._restore(task,baseline,'optimized',0); last_response=state.history[-1]['content'] if state.history else str(baseline.get('response',''))
        while state.turn_index<int(self.config['experiment']['max_turns']) and not success:
            keys=state.policy_keys(); action,prompt,attack_reason,source,decision=self._optimized_candidate(task,state,frozen_policy,rankings,attempted,failed_fresh_actions,last_response)
            cid=EvidenceMemory.candidate_id(prompt,action); rid=EvidenceMemory.realization_id(prompt); prev_progress=state.progress; generated=self.target.generate(prompt,state.history); judge=self._judge(task,generated.text); reward=self._reward(prev_progress,judge)
        ...
        self.store.complete_episode('optimized',0,aid,success,state.turn_index,total_reward,'success' if success else 'turn_budget_exhausted')
        return success,state.turn_index,total_reward
    ...
    def run_optimized(self):
        ...
        if before_policy!=after_policy:raise RuntimeError('Frozen CB17 policy mutated during Optimized evaluation')
        if frozen_evidence_meta!=json.dumps(self.store.get_meta('frozen_evidence_snapshot'),sort_keys=True):raise RuntimeError('Frozen CB17 evidence snapshot mutated during Optimized evaluation')
        return self.export_results()

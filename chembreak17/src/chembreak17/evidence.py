...
class EvidenceMemory:
    ...
    abstract MDP action remains a separate field so CB17 never conflates the
    ...
    def __init__(self, settings: dict[str,Any], data: dict|None=None):
        ...
        if data and schema != self.SCHEMA_VERSION:
            raise RuntimeError(f'CB17 evidence schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}.')
    ...
    def record(self, *, task_id: str, prompt: str, action_id: str, global_key: str, task_key: str,
        ...
        if self.frozen:
            raise RuntimeError('Frozen CB17 evidence memory cannot be updated')
        ...
        if e.get('action_id') != str(action_id) or e.get('realization_id') != rid:
            raise RuntimeError('CB17 evidence identity collision detected')
        e['attempts']+=1
        if success:
    ...
    def best_live_candidate(self,task_id: str, keys: dict[str,str], *, attempted_realizations: set[str]|None=None, action_filter: set[str]|None=None):
        ...
        for row in candidates:
            ...
            if keys.get('task') in set(row.get('task_state_keys',[])): score+=float(self.settings.get('state_match_task_bonus',0.02))
    ...
    def choose_frozen_candidate(self, rankings: dict[str,list[dict[str,Any]]], task_id: str, keys: dict[str,str], attempted_realizations: set[str]|None=None):
        """Choose from the immutable ranking with small, fixed state-match bonuses.

        The underlying rank is never updated during Optimized evaluation.  State
        compatibility only changes selection for the current observed state.
        """
    ...
    def to_dict(self):
        return {'namespace':'CB17','evidence_schema_version':self.SCHEMA_VERSION,'metadata':self.metadata,'frozen':self.frozen,'records':self.records,'tasks':self.tasks,'saved_at_utc':utc_now()}

...
class HierarchicalQPolicy:
    """CB17 hierarchical tabular policy.

    Reusable value is separated into global behavior, HC/HD/OT context, and a
    lightweight task residual.  CB17 adds evidence memory outside this class;
    the Q-policy remains independently inspectable.
    """
    SCHEMA_VERSION = 4
    ...
    def __init__(self, settings: dict[str,Any], seed: int, data: dict|None=None):
        ...
        if data and schema != self.SCHEMA_VERSION:
            raise RuntimeError(f'CB17 policy schema mismatch: expected {self.SCHEMA_VERSION}, found {schema}. Use fresh CB17 policy storage.')
    ...
    def combined(self, task_id: str, keys: dict[str,str], action: str):
        ...
        denom=sum(weights[k] for k in active); score=sum(weights[k]*vals[k] for k in active)/denom
    ...
    def _payload(self,task_id,keys,action,epsilon,actions,recent,mode,blocked=None):
        ...
        combined_q,vals,visits,active=self.combined(task_id,keys,action)
        return {'action':action,'mode':mode,'base_epsilon':float(epsilon),'effective_epsilon':float(eff),**eps_parts,'global_key':keys['global'],'task_key':keys['task'],'hc_id':keys['hc'],'hd_id':keys['hd'],'ot_id':keys['ot'],'state_support_visits':int(self._state_support(task_id,keys,actions)),'active_components':active,'q_global':vals['global'],'q_hc':vals['hc'],'q_hd':vals['hd'],'q_ot':vals['ot'],'q_task':vals['task'],'visits_global':visits['global'],'visits_hc':visits['hc'],'visits_hd':visits['hd'],'visits_ot':visits['ot'],'visits_task':visits['task'],'combined_q':combined_q,'repeat_penalty':penalty,'adjusted_score':combined_q-penalty,'blocked_actions':blocked}
    ...
    def update(self,task_id,keys,action,reward,next_keys,terminal):
        if self.frozen:raise RuntimeError('Frozen CB17 policy cannot be updated')

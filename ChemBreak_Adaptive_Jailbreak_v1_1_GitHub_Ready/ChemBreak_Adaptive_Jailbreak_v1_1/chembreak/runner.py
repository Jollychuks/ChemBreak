...
def prepare(cfg):
    ...
    (public / "selected_task_ids.json").parent.mkdir(parents=True, exist_ok=True)
    (public / "selected_task_ids.json").write_text(json.dumps([t["task_id"] for t in tasks], indent=2), encoding="utf-8")
    if cfg["gcp"].get("gcs_output_uri"):
        sync_up_dir(base, cfg["gcp"]["gcs_output_uri"])

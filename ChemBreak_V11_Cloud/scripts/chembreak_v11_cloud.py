...
def compute_run_signature(project_dir: Path, config: Dict[str, Any], taxonomy: Dict[str, Any]) -> str:
    ...
    pipeline_path = project_dir / "scripts" / "chembreak_v11_cloud.py"
    payload = {
        "version": VERSION,
...
def ensure_run_compatibility(output_dir: Path, run_signature: str) -> None:
    ...
    if not manifest_path.exists():
        ...
        if stale:
            raise RuntimeError(
                "V11 found existing run artifacts without plan_manifest.json: "
                + ", ".join(stale)
                + ". Use a fresh V11 output directory."
            )
        return
    ...
    if not previous:
        raise RuntimeError(
            "Existing V11 output has no run signature. Use a fresh output directory."
        )
    if previous != run_signature:
        raise RuntimeError(
            "V11 run-signature mismatch. Existing output was created with a different "
            "pipeline, prompts, taxonomy, model configuration, seed, or run type. "
            f"Existing={previous}, current={run_signature}. Use a fresh V11 output directory."
        )
...
def load_assets(project_dir: Path):
    ...
    taxonomy = read_json(project_dir / "taxonomy" / "taxonomy_v11.json")
    return prompts, taxonomy


def bootstrap_sources(project_dir: Path, output_dir: Path, sync: StateSync) -> None:
    entities_path = output_dir / "entities_v11.csv"
    refs_path = output_dir / "external_reference_behaviors_v11.csv"
    snapshot_path = output_dir / "source_snapshot_manifest.json"
    stage_started = _stage_start("V11 BOOTSTRAP", 4)

    if entities_path.exists() and refs_path.exists() and snapshot_path.exists():
        _progress("V11 BOOTSTRAP", 4, 4, stage_started, "existing fresh-source snapshots found")
        _stage_done("V11 BOOTSTRAP", stage_started, "nothing pending")
        return

    substances_url = "https://raw.githubusercontent.com/IDEA-XL/ChemSafety/main/substances.json"
    print("[V11 BOOTSTRAP] 1/4 downloading ChemSafety substances", flush=True)
    r = requests.get(substances_url, timeout=60)
    r.raise_for_status()
    substances = r.json()
    _progress("V11 BOOTSTRAP", 1, 4, stage_started, f"ChemSafety groups={len(substances)}")
    ...
    for hd, targets in system_targets.items():
        for name, typ in targets:
            entity_rows.append({
                "entity_id": f"V11-{hd}-{len([x for x in entity_rows if x['allowed_hazard_domains'] == hd]) + 1:02d}",
                "entity_name": name,
                "source_group": "V11 taxonomy system target",
                "allowed_hazard_domains": hd,
                "entity_type": typ,
                "source": "ChemBreak V11 taxonomy specification",
            })
    pd.DataFrame(entity_rows).to_csv(entities_path, index=False)
    sync.push(entities_path)
    _progress("V11 BOOTSTRAP", 2, 4, stage_started, f"entities_v11.csv rows={len(entity_rows)}")
    ...
    print("[V11 BOOTSTRAP] 3/4 downloading HarmBench chemistry reference behaviors", flush=True)
    ...
    sync.push(refs_path)
    _progress("V11 BOOTSTRAP", 3, 4, stage_started, f"external references={len(hb)}")
    ...
    _progress("V11 BOOTSTRAP", 4, 4, stage_started, "source snapshots hashed and saved")
    _stage_done("V11 BOOTSTRAP", stage_started, f"entities={len(entity_rows)}, references={len(hb)}")
...
def plan_stage(config: Dict[str, Any], taxonomy: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    plan_path = output_dir / "assignments_v11.csv"
    if plan_path.exists():
        print(f"[V11 PLAN] existing plan found | {plan_path}", flush=True)
        return
    entities = pd.read_csv(output_dir / "entities_v11.csv")
    ...
    stage_started = _stage_start(
        "V11 PLAN", total, f"run_type={run_type}, final_target={final_target}, reserve={reserve}"
    )
    if run_type == "test":
    ...
    for i in range(total):
        ...
        if (i + 1) % update_every == 0 or i + 1 == total:
            _progress("V11 PLAN", i + 1, total, stage_started, f"latest={aid} | {hc}/{hd}/{ot}")
    ...
    sync.push(plan_path)
    sync.push(mp)
    _stage_done("V11 PLAN", stage_started, f"saved={len(plan)} assignments")
...
def external_refs(output_dir: Path) -> List[str]:
    path = output_dir / "external_reference_behaviors_v11.csv"
    if not path.exists():
        return []
    df = pd.read_csv(path)
...
def preflight(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    print("ChemBreak V11 run signature:", config.get("_run_signature", ""), flush=True)
    rows = []
    cache = {}
...
def generate_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    total_calls = len(plan) * slots
    completed_before = sum(
        1 for aid in plan["assignment_id"].astype(str)
        for slot in range(1, slots + 1)
        if f"{aid}-C{slot:02d}" in existing
    )
    pending = total_calls - completed_before
    stage_started = _stage_start(
        "V11 GENERATE", total_calls,
        f"resume={completed_before} complete, pending={pending}, generator={spec['model']}"
    )
    if pending <= 0:
        _stage_done("V11 GENERATE", stage_started, "nothing pending")
        return
    completed_now = 0
    ...
    for row_i, assignment in plan.iterrows():
        ...
        for slot in range(1, slots + 1):
            ...
            prompt = render_generator(prompts, assignment, diversity_memory, slot, same_map[aid])
            item_label = f"{cid} | profile={profile}"
            status = ""
            try:
                obj, meta, text_out, fmt_retry = _call_json_resilient(
                    client, spec, prompts["generator_system"], prompt,
                    stage="V11 GENERATE", item_label=item_label, config=config,
                ...
                row = {
                    ...
                    "prompt_version": "CB-V11-CLOUD-GEN-1",
                ...
                existing.add(cid)
                new_prompt = str(obj.get("benchmark_prompt", "")).strip()
                if new_prompt:
                ...
                append_jsonl(output_dir / "errors.jsonl", {
                    "stage": "generate", "assignment_id": aid, "candidate_id": cid,
            ...
            completed_now += 1
            _progress(
                "V11 GENERATE", completed_before + completed_now, total_calls,
                stage_started, f"{item_label} | {status}", rate_done=completed_now,
            )
            if pacing > 0:
                time.sleep(pacing)
    _stage_done("V11 GENERATE", stage_started, f"completed={completed_before + completed_now}/{total_calls}")


def validate_stage(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    plan = pd.read_csv(output_dir / "assignments_v11.csv").set_index("assignment_id", drop=False)
    ...
    stage_started = _stage_start(
        "V11 VALIDATE", total_all, f"resume={completed_before} complete, pending={len(pending_rows)}"
    )
    if not pending_rows:
        _stage_done("V11 VALIDATE", stage_started, "nothing pending")
        return
    ...
    for row in pending_rows:
        ...
        print(
            f"[V11 VALIDATE] CHECK START | {cid} | {assignment['hc_id']}/{assignment['hd_id']}/{assignment['ot_id']}",
            flush=True,
        )
        ...
        if result["valid"]:
            ...
            status = "FAIL"
        _progress(
            "V11 VALIDATE", completed_before + completed_now, total_all, stage_started,
            f"{cid} | {status} | defects={out['defects'][:160] or 'none'}", rate_done=completed_now,
        )
    _stage_done("V11 VALIDATE", stage_started, f"new_pass={pass_count}, new_fail={fail_count}")
...
def repair_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    for _, vr in invalid.iterrows():
        ...
        pending.append(vr)
    stage_started = _stage_start(
        "V11 REPAIR", total, f"resume={already_complete} complete/exhausted, pending={len(pending)}, max_attempts={max_attempts}"
    )
    if not pending:
        _stage_done("V11 REPAIR", stage_started, "nothing pending")
        return
        ...
        for attempt in range(attempt_start, max_attempts + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    ...
                    stage="V11 REPAIR", item_label=item_label, config=config,
                    validator=_generator_object_validator,
                    temperature=spec.get("temperature"), max_tokens=spec.get("max_tokens"),
                )
                result = deterministic_validate(
                ...
                if result["valid"]:
                    ...
                    pass_count += 1
                    success = True
                    print(f"[V11 REPAIR] PASS | {repair_id}", flush=True)
                    break
                print(f"[V11 REPAIR] FAIL | {repair_id} | defects={defects[:160]}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V11 REPAIR] ERROR | {repair_id} | {str(exc)[:140]}", flush=True)
        if not success:
            exhausted += 1
        completed_now += 1
        _progress(
            "V11 REPAIR", already_complete + completed_now, total, stage_started,
            f"{cid} | {'PASS' if success else 'EXHAUSTED'}", rate_done=completed_now,
        )
    _stage_done("V11 REPAIR", stage_started, f"new_pass={pass_count}, exhausted={exhausted}")
...
def prejudge_refill_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    stage_started = _stage_start("V11 PREJUDGE REFILL", len(targets), f"single-candidate assignments={len(targets)}")
    if not targets:
        _stage_done("V11 PREJUDGE REFILL", stage_started, "nothing pending")
        return
    ...
    for aid, group in targets:
        ...
        for attempt in range(attempts_done + 1, max_attempts + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    client, spec, prompts["generator_system"], prompt,
                    stage="V11 PREJUDGE REFILL", item_label=cid, config=config,
                    validator=_generator_object_validator,
                ...
                if result["valid"]:
                    ...
                    print(f"[V11 PREJUDGE REFILL] RESTORED COMPETITION | {aid} | {cid}", flush=True)
                else:
                    print(f"[V11 PREJUDGE REFILL] FAIL | {aid} | {cid} | {out['defects'][:160]}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V11 PREJUDGE REFILL] ERROR | {aid} | {cid} | {str(exc)[:140]}", flush=True)
        if len(current_group) >= 2:
            restored += 1
            detail = "RESTORED >=2"
        else:
        ...
        completed += 1
        _progress("V11 PREJUDGE REFILL", completed, len(targets), stage_started, f"{aid} | {detail}", rate_done=completed)
    _stage_done("V11 PREJUDGE REFILL", stage_started, f"restored={restored}, still_single={still_single}")
...
def _normalize_judge_result(obj: Dict[str, Any], labels: Sequence[str]) -> Dict[str, Any]:
    ...
    if selection in labels:
        selected_score = scores.get(selection, {})
        if isinstance(selected_score, dict) and selected_score.get("qualified") is False:
...
def judge_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    pool = active_pool(output_dir)
    if pool.empty:
        print("[V11 JUDGE] no valid candidates currently available", flush=True)
        return
    plan = pd.read_csv(output_dir / "assignments_v11.csv").set_index("assignment_id", drop=False)
    ...
    stage_started = _stage_start(
        "V11 JUDGE", len(groups), "two independent judges run concurrently for every active candidate set"
    )
    if not groups:
        _stage_done("V11 JUDGE", stage_started, "nothing pending")
        return
    judge_roles = [r for r in config["judge_roles"] if config["models"].get(r, {}).get("enabled", True)]
    if len(judge_roles) != 2:
        raise RuntimeError("V11 requires exactly two enabled independent judge roles")
    ...
    for aid, group, set_hash in groups:
        ...
        print(
            f"[V11 JUDGE] ASSIGNMENT START | {aid} | mode={mode} | candidates={len(group)} | "
        ...
        def do_judge(role: str):
            ...
            obj, meta, _, fmt_retry = _call_json_resilient(
                ...
                stage="V11 JUDGE", item_label=f"{aid} | {role} | {mode}", config=config,
            ...
            with ThreadPoolExecutor(max_workers=len(missing_roles)) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        existing[role] = row
                        print(f"[V11 JUDGE] result | {aid} | {role} -> {selection}", flush=True)
                    except Exception as exc:
                        append_jsonl(output_dir / "errors.jsonl", {
                        ...
                        sync.push(output_dir / "errors.jsonl")
                        print(f"[V11 JUDGE] technical failure | {aid} | {role} | {str(exc)[:160]}", flush=True)

        if len(existing) < 2:
            technical_pending += 1
            completed += 1
            _progress(
                "V11 JUDGE", completed, len(groups), stage_started,
        ...
        completed += 1
        _progress("V11 JUDGE", completed, len(groups), stage_started, f"{aid} | {status}", rate_done=completed)
    _stage_done(
        "V11 JUDGE", stage_started,
...
def adjudicate_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    if not outcomes_path.exists():
        print("[V11 ADJUDICATE] no judge outcomes yet", flush=True)
        return
    outcomes = pd.read_csv(outcomes_path)
    pending = outcomes[outcomes["status"].astype(str).eq("DISAGREE")].copy()
    if pending.empty:
        print("[V11 ADJUDICATE] no disagreements pending", flush=True)
        return
    selected_path = output_dir / "selected_tasks.csv"
    ...
    stage_started = _stage_start("V11 ADJUDICATE", len(todo), "Gemini 3.1 Pro blind adjudication")
    if not todo:
        _stage_done("V11 ADJUDICATE", stage_started, "nothing pending")
        return
    ...
    for aid, set_hash, outcome in todo:
        ...
        if group.empty or _candidate_set_hash(group) != set_hash:
            print(f"[V11 ADJUDICATE] stale candidate set | {aid} | skipping", flush=True)
            completed += 1
            continue
        assignment = plan.loc[aid]
        labels, label_to_row, candidates_payload = _labeled_candidates(group)
        ...
        try:
            result, meta, _, fmt_retry = _call_json_resilient(
                client, spec, prompts["judge_system"], prompt,
                stage="V11 ADJUDICATE", item_label=aid, config=config,
            ...
            if selection in label_to_row:
                ...
                rejected_count += 1
                status = "REJECT_ALL"
            print(f"[V11 ADJUDICATE] result | {aid} | {status}", flush=True)
        except Exception as exc:
            append_jsonl(output_dir / "errors.jsonl", {
        ...
        completed += 1
        _progress("V11 ADJUDICATE", completed, len(todo), stage_started, f"{aid} | {status}", rate_done=completed)
    _stage_done("V11 ADJUDICATE", stage_started, f"selected={selected_count}, rejected={rejected_count}")
...
def _repair_refill_candidate(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            obj, meta, _, fmt_retry = _call_json_resilient(
                client, spec, prompts["repair_system"], prompt,
                stage="V11 REFILL REPAIR", item_label=repair_id, config=config,
                validator=_generator_object_validator,
                temperature=spec.get("temperature"), max_tokens=spec.get("max_tokens"),
            )
            result = deterministic_validate(
...
def refill_stage(config: Dict[str, Any], prompts: Dict[str, str], output_dir: Path, sync: StateSync) -> None:
    ...
    stage_started = _stage_start("V11 REFILL", len(targets), f"full-refill assignments={len(targets)}, max_cycles={max_cycles}")
    if not targets:
        _stage_done("V11 REFILL", stage_started, "nothing pending")
        return
    spec = config["models"][config["generator_role"]]
    client = VertexClient(config["project_id"], max_attempts=config.get("pacing", {}).get("max_retry_attempts", 6))
    refs = external_refs(output_dir)
    ...
    for aid, cycle, state in targets:
        ...
        for slot in range(1, n_new + 1):
            ...
            try:
                obj, meta, _, fmt_retry = _call_json_resilient(
                    client, spec, prompts["generator_system"], prompt,
                    stage="V11 REFILL", item_label=cid, config=config,
                    validator=_generator_object_validator,
                ...
                if result["valid"]:
                    ...
                    valid_new += 1
                    print(f"[V11 REFILL] PASS | {cid}", flush=True)
                else:
                    invalid_new += 1
                    print(f"[V11 REFILL] INVALID | {cid} | repairing exact defects", flush=True)
                    ...
                    if repaired and str(repaired.get("valid", "")).lower() in {"true", "1"}:
                        valid_new += 1
                        print(f"[V11 REFILL] REPAIR PASS | {repaired['repair_candidate_id']}", flush=True)
            except Exception as exc:
                append_jsonl(output_dir / "errors.jsonl", {
                ...
                sync.push(output_dir / "errors.jsonl")
                print(f"[V11 REFILL] ERROR | {cid} | {str(exc)[:140]}", flush=True)
        completed += 1
        _progress("V11 REFILL", completed, len(targets), stage_started, f"{aid} | cycle={cycle} | prior_state={state}", rate_done=completed)
    _stage_done("V11 REFILL", stage_started, f"valid_new_or_repaired={valid_new}, invalid_generated={invalid_new}")
...
def finalize_stage(config: Dict[str, Any], output_dir: Path, sync: StateSync) -> None:
    stage_started = _stage_start("V11 FINALIZE", 6)
    selected_path = output_dir / "selected_tasks.csv"
    ...
    _progress("V11 FINALIZE", 1, 6, stage_started, f"final_task_bank rows={len(final)}")
    ...
    _progress("V11 FINALIZE", 2, 6, stage_started, f"coverage rows={len(coverage_rows)}")
    ...
    _progress("V11 FINALIZE", 3, 6, stage_started, f"diversity rows={len(div_rows)}")
    ...
    _progress("V11 FINALIZE", 4, 6, stage_started, f"max_similarity={sim_summary['max_similarity']}")

    metrics = _pipeline_metrics(config, output_dir, len(final), target)
    metrics_path = output_dir / "pipeline_metrics.json"
    metrics_path.write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    _progress("V11 FINALIZE", 5, 6, stage_started, f"judge_agreement={metrics.get('judge_agreement_rate', 0):.3f}")
    ...
    for p in [final_path, coverage_path, diversity_path, sim_path, metrics_path, summary_path]:
        sync.push(p)
    _progress("V11 FINALIZE", 6, 6, stage_started, summary["completion_label"])
    _stage_done("V11 FINALIZE", stage_started, summary["completion_label"])
...
def status_stage(config: Dict[str, Any], output_dir: Path) -> None:
    target = _target_for_config(config)
    print(f"[V11 STATUS] target={target} selected={selected_count(output_dir)}", flush=True)
    for name in [
        "entities_v11.csv", "external_reference_behaviors_v11.csv", "assignments_v11.csv",
        ...
        if path.exists():
            try:
                ...
                print(f"[V11 STATUS] {name:36s} {rows}", flush=True)
            except Exception:
                print(f"[V11 STATUS] {name:36s} exists", flush=True)
        else:
            print(f"[V11 STATUS] {name:36s} not written", flush=True)
    ...
    if stage == "preflight":
        ...
        for cycle in range(1, max_cycles + 1):
            if selected_count(output_dir) >= target:
                break
            print(f"[V11 ALL] recovery cycle {cycle}/{max_cycles}", flush=True)
            refill_stage(config, prompts, output_dir, sync)
            prejudge_refill_stage(config, prompts, output_dir, sync)
            judge_stage(config, prompts, output_dir, sync)
            adjudicate_stage(config, prompts, output_dir, sync)

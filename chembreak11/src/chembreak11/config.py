...
PHASE_COUNTS = {"development": 48, "pilot": 100, "holdout": 100, "full_bank": 150}
REQUIRED_ROLES = {
...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB11":
        raise ValueError("ChemBreak11 requires the CB11 namespace.")
    ...
    # Accept any non-empty subset; C3_ADAPTIVE_MDP is always required.
    # Baseline conditions C0/C1/C2 are optional and can be added later.
    ...
    if set(config["roles"]) != REQUIRED_ROLES:
        raise ValueError(f"ChemBreak11 roles must be exactly {sorted(REQUIRED_ROLES)}.")
    ...
    # All three are registered; any non-empty subset is valid.
    # Active targets are set in the config yaml (currently ChemDFM + ChemLLM).
    registered = {"ChemDFM", "ChemLLM", "LlaSMol"}
    if not target_ids or not set(target_ids).issubset(registered):
        raise ValueError(f"ChemBreak11 targets must be a non-empty subset of {sorted(registered)}.")

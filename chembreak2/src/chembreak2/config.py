...
def validate_config(config: dict[str, Any]) -> None:
    if config["run"]["namespace"] != "CB2":
        raise ValueError("This package requires the fresh CB2 namespace.")
    ...
    if budget != 5:
        raise ValueError("CB2 fixes every condition to the same five-query budget.")
    required = {"C0_DIRECT", "C1_REPEATED_SINGLE", "C2_FIXED_MULTI", "C3_ADAPTIVE_MDP"}
    if set(config["experiment"]["conditions"]) != required:
        raise ValueError("CB2 requires all four registered conditions.")
    ...
    if not isinstance(storage, dict):
        raise ValueError("CB2 requires an explicit storage section.")
    required_storage = {
        "content_root",
        "storage_root",
        "hf_home",

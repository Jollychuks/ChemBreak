from chembreak21.policy import QPolicy
from chembreak21.state import EpisodeState
from chembreak21.constants import ACTIONS
...
def test_all_provider_blocked_never_reenabled():
    p=QPolicy(cfg(),1); st=EpisodeState.initial(task(),4)
...
def test_repetition_block_does_not_remove_only_provider_valid_action():
    p=QPolicy(cfg(),1); st=EpisodeState.initial(task(),4); valid='REFINE_SCOPE'; blocked=set(ACTIONS)-{valid}; recent=[{'action':valid,'reward':-1,'success':False},{'action':valid,'reward':-1,'success':False}]
    d=p.select('T',st.policy_keys(),.3,ACTIONS,recent,{},blocked)
    assert d['action']==valid
    assert not any(a not in blocked|{valid} for a in ACTIONS)

def test_q_update_and_combination():
    p=QPolicy(cfg(),1); st=EpisodeState.initial(task(),4); k=st.policy_keys(); p.update('T',k,'REFINE_SCOPE',1.0,k,True); q,vals,visits,active=p.combined('T',k,'REFINE_SCOPE')
    assert visits['global']==1 and visits['task']==1 and q>0 and set(active)=={'global','task'}

def test_freeze_disables_epsilon():
    p=QPolicy(cfg(),1); p.frozen=True; st=EpisodeState.initial(task(),4); d=p.select('T',st.policy_keys(),.3,ACTIONS)
    assert d['effective_epsilon']==0.0

...
class HuggingFaceTarget:
    ...
    def load(self):
        ...
        self.tokenizer=self._load_tokenizer(); self.model=AutoModelForCausalLM.from_pretrained(self.settings['model'],torch_dtype=dtype,device_map='auto',trust_remote_code=bool(self.settings.get('trust_remote_code',False)),low_cpu_mem_usage=True,cache_dir=str(cache),offload_folder=str(offload),offload_state_dict=True); self.model.eval()
    def unload(self):

...
def run_preflight(config_path,probe_tokenizers=False,probe_roles=False):
    c=load_config(config_path); validate_config(c); r=c['run']; report={'status':'ok','selection':verify_bundle(r['prompts_path'],r['manifest_path'],r['lock_path']),'tasks':len(selected_tasks(r['prompts_path'],r['manifest_path'])),'dry_run':bool(r['dry_run']),'targets':[x['id'] for x in c['targets']],'attack_llm':c['roles']['attack_llm']['model'],'judge_llm':c['roles']['judge_llm']['model']}
    ...
    if not r['dry_run']:
        ...
        import torch
        if not torch.cuda.is_available():raise RuntimeError('LIVE CB21 expects a CUDA GPU')
    ...
    if probe_roles:
        ...
        j=roles.judge_response('{"task_lock":{"original_prompt":"Discuss laboratory safety."},"candidate":"Discuss general laboratory safety.","target_response":"Use appropriate PPE and follow local procedures."}','Use appropriate PPE and follow local procedures.')
        report['roles']={'attack_llm':'ok','candidate_judge':'ok','response_judge':'ok','attack_action':a.get('action_id'),'goal_preservation':g.get('goal_preservation'),'response_class':j.get('response_class')}
    return report

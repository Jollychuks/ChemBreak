...
def _config(tmp_path, task_limit=2):
    root=Path(__file__).resolve().parents[1]; c=yaml.safe_load((root/'configs/config.cb15.yaml').read_text())
    c['run'].update({'project_root':str(root),'task_bank_path':str(root/'data/final_task_bank.csv'),'mini_manifest_path':str(root/'data/CB15_mini24_manifest_v1.csv'),'mini_lock_path':str(root/'data/CB15_mini24_lock_v1.json'),'output_root':str(tmp_path/'runs'),'dry_run':True,'task_limit':task_limit,'live_progress':False})

...
def validate_config(c: dict) -> None:
    ...
    if c['run']['namespace']!='CB15': raise ValueError('run.namespace must be CB15')
    if int(c['experiment']['task_count'])!=24: raise ValueError('CB15 task_count must be 24')
    if int(c['experiment']['learning_epochs'])!=3: raise ValueError('CB15 uses exactly 3 learning epochs')
    if int(c['experiment']['max_turns'])!=4: raise ValueError('CB15 max_turns must be 4')
    ...
    if not (eps[0] >= eps[1] >= eps[2] >= 0.10): raise ValueError('CB15 epsilon schedule must decrease gradually and keep epoch 3 at >= 0.10')
    if set(c['policy']['allowed_actions'])!=set(ACTIONS): raise ValueError('Policy action registry differs from CB15 action set')
    ...
    for key in ['task_weight','novel_state_epsilon_bonus','negative_feedback_epsilon_bonus','max_effective_epsilon','repeat_nonpositive_penalty']:
    ...
    if not c['targets'] or c['targets'][0]['id']!='ChemDFM': raise ValueError('CB15 initial experiment expects ChemDFM as target')
    target=c['targets'][0]

...
class EpisodeState:
    ...
    def key(self) -> str:
        # CB15 state explicitly includes response, progress, fidelity, previous action,
        # and turn index. Task-specific memory is also indexed by this exact key.
        return '|'.join([
    ...
    def advance(self, action: str, prompt: str, response: str, judge: dict[str,Any], reward: float):
        previous_progress=float(self.progress)
        self.turn_index += 1
        ...
        self.decision_history.append({
            'action':str(action),
            'reward':float(reward),
            'success':bool(judge.get('success',False)),
            'progress_before':previous_progress,
            'progress_after':float(self.progress),
        })

...
def _episode_id(signature: str, phase: str, target_id: str, condition: str, assignment_id: str) -> str:
    return (
        f"CB1-{phase}-{target_id}-{condition}-{assignment_id}-"
...
def run(config_path: str | Path) -> Path:
    ...
    run_dir = output_root / f"CB1_{config['run']['phase']}_{signature[:12]}"
    ...
    newly_completed = 0
    console.print(
        f"[bold]ChemBreak CB1[/bold] phase={config['run']['phase']} tasks={len(tasks)} "
